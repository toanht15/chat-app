...
io.sockets.on('connection', function(socket) {
  ...
  var chatApi = {
    ...
    sendUnreadCnt: function(evName, obj, toUserFlg) {
      var sql, ret = {tabId: obj.tabId, sincloSessionId: obj.sincloSessionId, chatUnreadId: null, chatUnreadCnt: 0},
        sincloSessionId = obj.sincloSessionId, siteId = companyList[obj.siteKey];
          ...
          if (type == 1 && rows[0].display_type === 3) {
            ...
            if (active_flg == 1) {
              for (var i2 = 0; i2 < common.publicHolidaySettingsArray.length; i2++) {
                //祝日の場合
                if ((now.getFullYear() + "/" + "0" + (now.getMonth() + 1)).slice(-2) + '/' + ("0" + (now.getDate())).slice(-2) == common.publicHolidaySettingsArray[i2].year + '/' + common.publicHolidaySettingsArray[i2].month + '/' + common.publicHolidaySettingsArray[i2].day) {
                  //祝日の営業時間設定が「休み」でない場合
      ...
      .then((text) => {
        ...
        if (apiCaller.isSwitchingOperator()) {
          ...
          if (obj.messageRequestFlg == 1) {
            ...
            pool.query('SELECT id FROM t_history_chat_logs WHERE visitors_id = ? and t_histories_id = ? and message_distinction = ? and message_type = 998', [obj.userId, obj.historyId, obj.messageDistinction], function(err, results) {
              if (Object.keys(results) && Object.keys(result).length !== 0) {
                obj.messageRequestFlg = 0;
              }
              chatApi._handleInsertData(null, customerMessageInsertResult, obj, true, customerSendData);
            });
          }
          else {
            chatApi._handleInsertData(null, customerMessageInsertResult, obj, true, customerSendData);
      ...
      if (results !== null) {
        ...
        if (functionManager.isEnabled(obj.siteKey, functionManager.keyList.useCogmoAttendApi)
          ...
          if (!isset(sincloCore[obj.siteKey][obj.sincloSessionId].apiCaller)) {
            ...
            sincloCore[obj.siteKey][obj.sincloSessionId].apiCaller.init(sincloCore[obj.siteKey][obj.tabId].historyId, obj, fullDateTime(), emit).then(() => {
