...
class MUser extends AppModel
{
  ...
  public function isCurrentPw($currentPw)
  {
    ...
    if (empty($currentPw['current_password'])) {
      return false;
    }
  ...
  public function notOverlapMail()
  {
    ...
    if (!empty($data['new_password'])) {
      if (strpos($data['new_password'],
          (substr($data['mail_address'], 0, strpos($data['mail_address'], '@')))) === false) {
        return true;
      }
      return false;
    }
  ...
  public function incrementErrorCount($target)
  {
    ...
    $data = array(
      'id' => $target['id'],
      'error_count' => $target['error_count'] + 1,
      'locked_datetime' => $target['locked_datetime']
    );
