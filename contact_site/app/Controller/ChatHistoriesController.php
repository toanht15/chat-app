...
//          'THistoryChatLog.hide_flg' => 0,
      ...
      foreach ($userList as $val) {
        ...
        if ((isset($val['THistoryChatLog2']['type']) && isset($infoData['History']['chat_type']) &&
          ...
          if ($val['THistoryChatLog']['message_type'] == 12
            || $val['THistoryChatLog']['message_type'] == 33
            || $val['THistoryChatLog']['message_type'] == 34
            || $val['THistoryChatLog']['message_type'] == 35
            || $val['THistoryChatLog']['message_type'] == 43
            || $val['THistoryChatLog']['message_type'] == 47
            || $val['THistoryChatLog']['message_type'] == 50
            || $val['THistoryChatLog']['message_type'] == 53) {
            $row['transmissionKind'] = '訪問者（ヒアリング回答）';
            $row['transmissionPerson'] = '';
          }
          if ($val['THistoryChatLog']['message_type'] == 53) {
            $json = json_decode($val['THistoryChatLog']['message'], TRUE);
          ...
          if ($val['THistoryChatLog']['message_type'] == 36
            || $val['THistoryChatLog']['message_type'] == 37
            || $val['THistoryChatLog']['message_type'] == 38
            || $val['THistoryChatLog']['message_type'] == 39
          ...
          if ($val['THistoryChatLog']['message_type'] == 41
            || $val['THistoryChatLog']['message_type'] == 42
            || $val['THistoryChatLog']['message_type'] == 45
            || $val['THistoryChatLog']['message_type'] == 46
            || $val['THistoryChatLog']['message_type'] == 49
            || $val['THistoryChatLog']['message_type'] == 52
            || $val['THistoryChatLog']['message_type'] == 55) {
            ...
            $row['transmissionKind'] = 'シナリオメッセージ（ヒアリング：' . $typeLabel . '）';
            $row['transmissionPerson'] = $this->userInfo['MCompany']['company_name'];
            if (!$json->message) {
              $val['THistoryChatLog']['message'] = '';
            } else {
            ...
            } else if($val['THistoryChatLog']['message_type'] == 52) {
              if (!$json->message) {
                $val['THistoryChatLog']['message'] .= "\n";
              }
              foreach($json->settings->options as $idx => $option) {
          ...
          if ($val['THistoryChatLog']['message_type'] == 300) {
            ...
            $row['transmissionKind'] = 'チャットツリーメッセージ（分岐：' . $typeLabel . '）';
            $row['transmissionPerson'] = $this->userInfo['MCompany']['company_name'];
            if (!$json->message) {
              $val['THistoryChatLog']['message'] = "";
            } else {
            ...
            foreach($json->labels as $idx => $assoc) {
              if($idx === (count($json->labels) - 1)) {
                $val['THistoryChatLog']['message'] .= ($assoc->type === '1' ? '[] ' : '').$assoc->value;
              } else {
                $val['THistoryChatLog']['message'] .= ($assoc->type === '1' ? '[] ' : '').$assoc->value."\n";
              }
            }
          ...
          foreach ($ret as $val) {
            ...
            switch ($val['THistoryChatLog']['message_type']) {
              ...
              case 23: // シナリオメッセージ（選択肢）
                ...
                break;
              case 81: // CogmoAttend チャットボットメッセージ
              case 82: // CogmoAttend チャットボットメッセージ（フィードバック）
                $row = $this->_setData($date, "自動返信", $this->userInfo['MCompany']['company_name'], $message);
                break;
              case 998: // 入室メッセージ
              case 999: // 退室メッセージ
  ...
  private function _setList($type = true)
  {
    ...
    if ($this->Session->check('Thistory')) {
      ...
      if (!empty($data['History']['start_day'])) {
        $this->paginate['THistory']['conditions']['AND']['THistory.firstSpeechTime >='] = str_replace('/', '-', $data['History']['start_day']).' 00:00:00';
      }
      //終了日
      if (!empty($data['History']['finish_day'])) {
        $this->paginate['THistory']['conditions']['AND']['THistory.firstSpeechTime <='] = str_replace('/', '-', $data['History']['finish_day']).' 23:59:59';
      }
      ...
      if (isset($data['THistoryChatLog']['responsible_name']) && $data['THistoryChatLog']['responsible_name'] !== "") {
        ...
        if (strpos($data['THistoryChatLog']['responsible_name'], '%') !== false) {
          $data['THistoryChatLog']['responsible_name'] = str_replace('%', '\\%', $data['THistoryChatLog']['responsible_name']);
        }
    ...
    if ($this->coreSettings[C_COMPANY_USE_CHAT]) {
      ...
      if (!$hasCustomDataCondition && empty($data['History']['chat_type']) && empty($data['History']['ip_address']) && empty($data['THistoryChatLog']['responsible_name'])
        ...
        $chatStateList = $dbo2->buildStatement(
          [
            'table' => "(SELECT t_histories_id,t_history_stay_logs_id,m_companies_id,message_type,notice_flg,created,message_read_flg, COUNT(*) AS count, SUM(CASE WHEN achievement_flg = -1 THEN 1 ELSE 0 END) terminate, SUM(CASE WHEN achievement_flg = 0 THEN 1 ELSE 0 END) cv, SUM(CASE WHEN achievement_flg = 1 THEN 1 ELSE 0 END) deny, SUM(CASE WHEN achievement_flg = 2 THEN 1 ELSE 0 END) eff, SUM(CASE WHEN message_type = 998 THEN 1 ELSE 0 END) cmp,SUM(CASE WHEN notice_flg = 1 THEN 1 ELSE 0 END) notice,SUM(CASE WHEN message_type = 3 THEN 1 ELSE 0 END) auto_message,SUM(CASE WHEN message_type = 4 THEN 1 ELSE 0 END) sry, SUM(CASE WHEN (message_type = 1 OR message_type = 303) THEN 1 ELSE 0 END) cus,SUM(CASE WHEN message_type = 1 AND message_read_flg = 0 THEN 1 ELSE 0 END) unread, SUM(CASE WHEN message_type = 5 THEN 1 ELSE 0 END) auto_speech, SUM(CASE WHEN message_type >= 12 AND message_type <= 13 THEN 1 ELSE 0 END) se_cus, SUM(CASE WHEN message_type >= 21 AND message_type <= 27 THEN 1 ELSE 0 END) se_auto FROM t_history_chat_logs AS THistoryChatLog force index(idx_t_history_chat_logs_m_companies_id_t_histories_id_created) WHERE `THistoryChatLog`.m_companies_id = " . $this->userInfo['m_companies_id'] . " AND `t_histories_id` = `THistoryChatLog`.t_histories_id GROUP BY t_histories_id ORDER BY t_histories_id desc)",
            'alias' => 'chat',
            'fields' => [
              'chat.*',
      ...
      $this->THistory->virtualFields['firstSpeechTime'] = 'SpeechTime.firstSpeechTime';
     ...
     //有人チャット受信日時
      ...
      if (isset($data['THistoryChatLog']['send_chat_page']) && $data['THistoryChatLog']['send_chat_page'] !== "") {
        //「%」が含まれていた場合
        if (strpos($data['THistoryChatLog']['send_chat_page'], '%') !== false) {
          $data['THistoryChatLog']['send_chat_page'] = str_replace('%', '\\%', $data['THistoryChatLog']['send_chat_page']);
        }
  ...
  private function _searchConditions()
  {
    ...
    if (!empty($data['History']['start_day'])) {
      $conditions[] = [
        'THistory.firstSpeechTime >=' => str_replace('/', '-', $data['History']['start_day']).' 00:00:00'
      ];
    }
    ...
    if (!empty($data['History']['finish_day'])) {
      $conditions[] = [
        'THistory.firstSpeechTime <=' => str_replace('/', '-', $data['History']['finish_day']).' 23:59:59'
      ];
    }
      ...
      $chatStateList = $dbo2->buildStatement(
        [
          ...
          'fields' => [
            ...
            'SUM(CASE WHEN (message_type = 1 OR message_type = 303) THEN 1 ELSE 0 END) cus'
          ],
          'order' => 't_histories_id',
          'group' => 't_histories_id'
        ],
