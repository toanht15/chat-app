...
<div id='tcustomerinformationsettings_idx' class="card-shadow">
  ...
  <div id='tcustomvaliables_menu' style= 'padding-left: 20px;'>
    <ul class="fLeft" >
        ...
        <span>
          <a>
            <?= $this->Html->image('add.png', array(
                'alt' => '登録',
                'id' => $coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS] ? "tcustomerinformationsettings_add_btn" : "tcustomerinformationsettings_disable_btn",
                'class'=>'btn-shadow'.($coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS] ? " btn-shadow disOffgreenBtn commontooltip" : " disOffgrayBtn  commontooltip disabled"),
                'data-text' => $coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS] ? "新規追加" : "こちらの機能はスタンダードプラン<br>からご利用いただけます。",
                'disabled' => !$coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS],
                'width' => 45,
                'height' => 45,
                'onclick' => $coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS] ? "openAddDialog()" : ""
            )) ?>
          </a>
        </span>
        <span>
      ...
      <div class="tabpointer">
        <label class="pointer">
          <?= $this->Form->checkbox('sort', array('onchange' => $coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS] ? "toggleSort()" : "")); ?><span id="sortText">並び替え</span><span id="sortTextMessage" style="display: none; font-size: 1.1em; color: rgb(192, 0, 0); font-weight: bold; ">（！）並び替え中（保存する場合はチェックを外してください）</span>
        </label>
      </div>
      <!-- 訪問ユーザー情報の並び替えモード -->
    </ul>
  ...
  <div id='tcustomerinformationsettings_list' class="p20x">
    <table>
      <thead>
      <tr>
        <th width=" 5%"><input type="checkbox" name="allCheck" id="allCheck" <?php if(isset($coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS]) && !$coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS]){ ?> disabled = "disabled" <?php } ?>><label for="allCheck"></label></th>
        <th width=" 5%">No</th>
    ...
    <tbody class="sortable">
      ...
      <?php foreach((array)$tCustomerInformationSettingList as $key => $val):?>
        <tr class="pointer" data-id="<?=$val['TCustomerInformationSetting']['id']?>" data-sort="<?=$val['TCustomerInformationSetting']['sort']?>" <?php if(isset($coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS]) && $coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS]){ ?> onclick="openEditDialog('<?=$val['TCustomerInformationSetting']['id']?>')" <?php } ?>>
          <!-- この記述が無いとチェックボックスをクリックしてもedit画面が開いてしまう -->
        <td width="5%" class="tCenter" onclick="event.stopPropagation()">
          <input type="checkbox" name="selectTab" id="selectTab<?=$key?>" value="<?=$val['TCustomerInformationSetting']['id']?>" <?php if(isset($coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS]) && !$coreSettings[C_COMPANY_USE_EDITCUSTOMERINFOMATIONS]){ ?> disabled = "disabled" <?php } ?>>
