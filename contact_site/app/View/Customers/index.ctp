                        ...
                        <td class="w10 tCenter" id="chatTypeBtn">
                            <p>
                              <ng-show="monitor.widget">
                                <span ng-click="ngChatApi.connect(monitor)" class="btn-shadow blueBtn " ng-if="!isset(monitor.chat)">チャット対応
                                  <div class="unread" ng-if="monitor.chatUnread.cnt > 0">{{monitor.chatUnread.cnt}}</div>
                                </span>
                                <span ng-click="ngChatApi.disConnect(monitor)" class="btn-shadow redBtn " ng-if="monitor.chat === <?= h($muserId)?>">チャット対応終了
                                  <div class="unread" ng-if="monitor.chatUnread.cnt > 0">{{monitor.chatUnread.cnt}}</div>
                                </span>
                                <span ng-if="isset(monitor.chat) && monitor.chat !== <?= h($muserId)?>">{{userList[monitor.chat]}}さんチャット対応中</span>
                              </ng-show>
                            </p>
                            <p>
                              <span ng-show="monitor.widget">
                                <a   ng-if="!monitor.connectToken" class='monitorBtn blueBtn btn-shadow' href='javascript:void(0)' ng-click="windowOpen(monitor.tabId)" ng-confirm-click="アクセスID【{{monitor.accessId}}】のユーザーに接続しますか？">画面同期を開始する</a>
                              </span>
