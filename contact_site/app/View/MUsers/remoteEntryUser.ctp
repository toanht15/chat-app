<script type="text/javascript">
  ...
  <?php $this->request->data['MUser']['display_name'] = htmlspecialchars($this->request->data['MUser']['display_name'],
    ENT_QUOTES, 'UTF-8');?>
  <?php $this->request->data['MUser']['mail_address'] = htmlspecialchars($this->request->data['MUser']['mail_address'],
    ENT_QUOTES, 'UTF-8');?>
  <?php $this->request->data['MUser']['password'] = htmlspecialchars($this->request->data['MUser']['password'],
    ENT_QUOTES, 'UTF-8');?>
  <?php $this->request->data['MUser']['permission_level'] = htmlspecialchars($this->request->data['MUser']['permission_level'],
    ENT_QUOTES, 'UTF-8');?>
...
<div class="form01" style="display:flex; flex-direction: column;">
    ...
    <div class="profile_icon_register" style="display:flex; flex-direction: column; align-items: center;">
      <div>
        ...
        <?php if (empty($this->request->data['MUser']['profile_icon'])) { ?>
          <i class="fa-user fal hover-changer"
      ...
      <div id="profile_register_btn" style="width: 100px">
        <div class="greenBtn btn-shadow icon_register"
             style="height: 25px; display: flex; justify-content: center; align-items: center; position: relative; overflow: hidden;"><?php echo $this->Form->file('uploadProfileIcon'); ?>
          写真を変更する
        </div>
        <div id="setToDefault" class="greenBtn btn-shadow icon_register" onclick="confirmToDefault()"
             style="height: 25px; display: flex; justify-content: center; align-items: center;">標準に戻す
        </div>
  ...
  <div class="grid_item">
    ...
    <?= $this->Form->input('permission_level', array(
      'type' => 'select',
