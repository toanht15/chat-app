...
<!--
 ...
 -->
        <ul id="chatTalk" class="details" ng-class="{ middleSize: showWidgetType === 1 && widgetSizeTypeToggle === '2',largeSize: showWidgetType === 1 && widgetSizeTypeToggle === '3'}">
...
<!-- 仕様変更、常に高度な設定が当たっている状態とする -->
<!--
        <span id="sincloAccessInfo" ng-hide="showWidgetType === 3" ng-class="{ notNone:widget_inside_border_none === ''||widget_inside_border_none === false,details: color_setting_type === '1' || color_setting_type === true }">ウェブ接客コード：●●●●</span>
 -->
        <span id="sincloAccessInfo" ng-hide="showWidgetType === 3" class="details" ng-class="{ notNone:widget_inside_border_none === ''||widget_inside_border_none === false }">ウェブ接客コード：●●●●</span>
        <?php endif; ?>
      </section>
    <?php endif; ?>
    <?php if ( $coreSettings[C_COMPANY_USE_SYNCLO] || (isset($coreSettings[C_COMPANY_USE_DOCUMENT]) && $coreSettings[C_COMPANY_USE_DOCUMENT]) ) :?>
