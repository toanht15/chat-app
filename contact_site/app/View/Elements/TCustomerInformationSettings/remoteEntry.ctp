    ...
    ?>
      <span id="BanedType1Label">
        <label class="forcheckbox <?php if((($count >= 3)&&$uncheckedflg))echo "grayfont commontooltip"?>" data-text="一覧表示に登録できるのは3つまでです。" for="TCustomerInformationSettingShowRealtimeMonitorFlg">
          ...
          この項目を<?php echo($coreSettings[C_COMPANY_ENABLE_REAL_TIME_MONITOR] ? 'リアルタイムモニタ' : '有人応対') ?>や履歴の一覧に表示する
        </label>
        <div class="questionBallon">
          <icon class="questionBtn commontooltip"
                data-text="<?php echo($coreSettings[C_COMPANY_ENABLE_REAL_TIME_MONITOR] ? 'リアルタイムモニタ' : '有人応対') ?>やチャット履歴の一覧画面に表示させる場合にチェックをしてください。（一覧に表示できる項目は最大で３つまでとなります。）">?</icon>
        </div>
      </span>
    </div>
    <div>
