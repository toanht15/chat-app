...
window.onload = function(){
  ...
  //追加ボタンの準備
  document.getElementById("tdocument_add_btn").addEventListener('click', openAdd, false);
...
function openConfirmDialog(){
  ...
  popupEvent.closePopup = toExecutableOnce(function(){
    $.ajax({
      ...
      url: "<?= $this->Html->url('/TDocuments/remoteDeleteDocuments') ?>",
      success: function(){
        $(".p-confirm #popup-button a").prop("disabled", true);
...
function openAdd(){
  ...
  if (!document.getElementById("sort").checked) {
    var url = "<?= $this->Html->url('/TDocuments/add') ?>";
    location.href = url;
  }
  else{
    return false;
