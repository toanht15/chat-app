...
<?php if ( $widgetCheck ){ ?>
  <?php if(strcmp($userInfo['permission_level'], C_AUTHORITY_NORMAL) !== 0): ?>
    ...
    <span>（<a href="" id="showOperatorPresenceLink" ng-click='showOperatorPresence()' class="commontooltip" data-text-center="true" data-balloon-width="210" data-balloon-position="48" data-text="オペレータステータス一覧を開く">待機中の人数：{{oprCnt}}人／離席中の人数：{{oprWaitCnt-oprCnt}}人</a>）</span>
    <?php else: ?>
...
<?php } else { ?>
  <?php if(strcmp($userInfo['permission_level'], C_AUTHORITY_NORMAL) !== 0): ?>
    <?php if(isset($coreSettings[C_COMPANY_USE_OPERATOR_PRESENCE_VIEW]) && $coreSettings[C_COMPANY_USE_OPERATOR_PRESENCE_VIEW]): ?>
      <span>（<a href="" id="showOperatorPresenceLink" ng-click='showOperatorPresence()' class="commontooltip" data-text-center="true" data-balloon-width="210" data-balloon-position="48" data-text="オペレータステータス一覧を開く">待機中のオペレータ人数：{{oprWaitCnt}}人</a>）</span>
    <?php else: ?>
