...
<?php if ( $widgetCheck ){ ?>
  <?php if(strcmp($userInfo['permission_level'], C_AUTHORITY_NORMAL) !== 0): ?>
    ...
    <span>（<a href="" id="showOperatorPresenceLink" ng-click='showOperatorPresence($event)' class="commontooltip ttposition_top" data-text-center="true" data-balloon-width="210" data-balloon-position="48" data-text="オペレータステータス一覧を開く">待機中の人数：{{oprCnt}}人／離席中の人数：{{oprWaitCnt-oprCnt}}人</a>）</span>
    <?php else: ?>
