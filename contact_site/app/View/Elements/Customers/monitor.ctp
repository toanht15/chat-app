...
<?php if ( $widgetCheck ){ ?>
  <?php if(strcmp($userInfo['permission_level'], C_AUTHORITY_NORMAL) !== 0): ?>
    ...
    <?php else: ?>
    <span class="commontooltip underL" data-balloon-width='278' data-text="オペレータステータス確認機能は<br>スタンダードプランからご利用いただけます。">（待機中の人数：{{oprCnt}}人／離席中の人数：{{oprWaitCnt-oprCnt}}人）</span>
    <?php endif; ?>
  <?php else: ?>
...
<?php } else { ?>
  <?php if(strcmp($userInfo['permission_level'], C_AUTHORITY_NORMAL) !== 0): ?>
    <?php if(isset($coreSettings[C_COMPANY_USE_OPERATOR_PRESENCE_VIEW]) && $coreSettings[C_COMPANY_USE_OPERATOR_PRESENCE_VIEW]): ?>
      <span class="commontooltip" data-text-center="true" data-balloon-width="210" data-balloon-position="48" data-text="オペレータステータス一覧を開く">（<a href="#" ng-click='showOperatorPresence()'>待機中のオペレータ人数：{{oprWaitCnt}}人</a>）</span>
    <?php else: ?>
      <span class="commontooltip underL" data-balloon-width='278' data-text="オペレータステータス確認機能は<br>スタンダードプランからご利用いただけます。">（待機中のオペレータ人数：{{oprWaitCnt}}人）</span>
    <?php endif; ?>
  <?php else: ?>
