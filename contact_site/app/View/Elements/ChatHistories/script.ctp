<script type="text/javascript">
  ...
  function customerInfoSave(historyId) {
    ...
    $.ajax({
      ...
      success: function(data) {
        location.href = "<?= $this->Html->url(['controller' => 'ChatHistories', 'action' => 'index']) ?>?id=" +
            historyId + '&edit';
      }
    });
  }
  ...
  $(function() {
    ...
    var calcHeaderHeight = function() {
      return $('#history_menu').outerHeight() + $('div.btnSet').outerHeight() + $('label[for="g_chat"]').outerHeight() +
      ...
      success: function(html) {
        ...
        if (customerData.tHistoryChatSendingPageData !== null) {
          document.getElementById(
              'chatSendingPage').innerHTML = customerData.tHistoryChatSendingPageData.FirstSpeechSendPage.title;
        ...
        for (var i = 0; i < customerInfoSettings.length; i++) {
          if (customerData.informations && customerData.informations[customerInfoSettings[i].item_name]) {
            $('#ng-customer-custom-' + customerInfoSettings[i].id).
                get(0).value = customerData.informations[customerInfoSettings[i].item_name];
          } else {
  ...
  function SetListHeight(type) {
    ...
    if (type == 1) {
      ...
      var adjustHeight = $('#history_menu').height() + $('.fLeft').outerHeight() +
          $('.dataTables_scrollHead').outerHeight() - 3;
