        ...
        <?php if(!empty($defaultHistoryList) && !empty($tHistoryCountData)) { ?>
          <ul>
            ...
            for($i = 0; $i < count($customerInformationList); $i++) {
              $informations = json_decode($mCusData['MCustomer']['informations'], TRUE);
              ...
              echo $this->htmlEx->visitorInput($customerInformationList[$i], false, true, false, $informations[$customerInformationList[$i]['item_name']]);
              echo '</li>';
            }
            ?>
          </ul>
