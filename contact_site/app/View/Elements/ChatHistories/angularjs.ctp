                ...
                if ($('#showChatTab .on').text() == '過去のチャット' && $('.switch .on').text().trim() != '詳細情報') {
                  $.ajax({
                    ...
                    url: "<?= $this->Html->url(array(
                      'controller' => 'ChatHistories',
                      'action' => 'remoteGetChatList'
                    )) ?>",
                    cache: false,
                    data: {
                      userId: $('#visitorsId').text()
      ...
      $scope.createTextOfMessage = function(chat, message, opt) {
        ...
        for (var i = 0; strings.length > i; i++) {
          ...
          if (option.radio && (radio > -1 || selectedRadio > -1)) {
            ...
            str = '<input type=\'radio\' name=\'' + radioName + '\' id=\'' + radioName + '-' + i +
                '\' class=\'sinclo-chat-radio\' value=\'' + val + '\' disabled=\'\'>';
            str += '<label class=\'pointer\' for=\'' + radioName + '-' + i + '\'>' + val + '</label>';
          }
          // チェックボックス
          var checkbox = str.indexOf('■');
          if (option.radio && (checkbox > -1)) {
            var val = '';
            if (checkbox > -1) {
              val = str.slice(checkbox + 2);
            }
            str = '<input type=\'checkbox\' name=\'' + radioName + '\' id=\'' + radioName + '-' + i +
                '\' class=\'sinclo-chat-checkbox\' value=\'' + val + '\' disabled=\'\'>';
            str += '<label class=\'pointer\' for=\'' + radioName + '-' + i + '\'>' + val + '</label>';
          }
          //リンク、電話番号
      ...
      $scope.createMessage = function(elem, chat) {
        ...
        if (type === chatApi.messageType.customer) {
          ...
          if (chat.delete_flg == 1) {
            ...
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important;font-size:' + fontSize + '\'>ゲスト(' +
            ...
            content = '<span class=\'cName\' style = \'color:#333333 !important; font-size:' + fontSize + '\'>ゲスト(' +
                Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'font-size:' + timeFontSize + '\'>' + chat.created + '</span>';
            if (chat.permissionLevel == 1 && coreSettings == 1) {
              content += '<img src= /img/close_b.png alt=履歴削除 onclick = openChatDeleteDialog(' + chat.id + ',' +
                  chat.t_histories_id + ',"' + forDeletionMessage + '","' + created +
              ...
              content += '<img src= /img/close_b.png alt=履歴削除 class = \"commontooltip disabled deleteChat\" data-text= \"こちらの機能はスタンダードプラン<br>からご利用いただけます。\" data-balloon-position = \"' +
                  dataBaloon +
                  '\"  width=21 height=21 style="cursor:pointer; float:right; color: #fff !important; padding:2px !important; margin-right: auto;">';
            }
          ...
          if (chat.delete_flg == 1) {
            ...
            content += '<span class=\'cChat\' style = \'font-size:' + fontSize + '\'>（「' +
                $scope.createTextOfMessage(chat, message, {radio: false}) + '」をクリック）</span>';
        ...
        // オートメッセージの場合
          ...
          if (chat.delete_flg == 1) {
            ...
            if (chat.permissionLevel == 1 && coreSettings == 1) {
              ...
              content += '<img src= /img/close_b.png alt=履歴削除 class = \"commontooltip disabled deleteChat\" data-text= \"こちらの機能はスタンダードプラン<br>からご利用いただけます。\" data-balloon-position = \"' +
                  dataBaloon +
                  '\"  width=21 height=21 style="cursor:pointer; float:right; color: #C9C9C9 !important; padding:2px !important; margin-right: auto;">';
            }
            content += '<span class=\'cChat\' style = \'font-size:' + fontSize + '\'>' +
                $scope.createTextOfMessage(chat, message) + '</span>';
          }
        } else if (type === chatApi.messageType.auto || type === chatApi.messageType.sorry || type ===
            chatApi.messageType.cogmo.message || type === chatApi.messageType.cogmo.feedback) {
          cn = 'sinclo_auto';
          var created = chat.created.replace(' ', '%');
          var forDeletionMessage = chat.message.replace(/\r?\n?\s+/g, '');
          if (message.indexOf('<') > -1) {
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize + '\'>自動応答(' +
                Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize + '\'>自動返信(' +
                Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important; font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
...
//        }
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize + '\'>ファイル送信' +
            ...
            content = '<span class=\'cName\' style = \'font-size:' + fontSize + '\'>ファイル送信' +
                (isExpired ? '（ダウンロード有効期限切れ）' : '') + '</span>';
            content += '<span class=\'cTime\' style = \'font-size:' + timeFontSize + '\'>' + chat.created + '</span>';
            if (chat.permissionLevel == 1 && coreSettings == 1) {
              content += '<img src= /img/close_b.png alt=履歴削除 width=21 height=21 onclick = openChatDeleteDialog(' +
            ...
            var isExpired = Math.floor((new Date()).getTime() / 1000) >=
                (Date.parse(message.expired.replace(/-/g, '/')) / 1000);
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important;font-size:' + fontSize +
                '\'>ゲスト（ヒアリング回答）(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important; font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important;font-size:' + fontSize +
                '\'>シナリオメッセージ（ヒアリング再回答）(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important; font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>シナリオメッセージ（テキスト発言）(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>シナリオメッセージ（ヒアリング）(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
          ...
          switch(type) {
            ...
            case chatApi.messageType.scenario.message.checkbox:
              typeLabel = 'チェックボックス';
              break;
            case chatApi.messageType.scenario.message.button:
              typeLabel = 'コンファーム';
              break;
          }
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>シナリオメッセージ（ヒアリング：' + typeLabel + '）(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
          } else {
            content = '<span class=\'cName\' style = \'font-size:' + fontSize + '\'>シナリオメッセージ（ヒアリング：' + typeLabel + '）</span>';
            content += '<span class=\'cTime\' style = \'font-size:' + timeFontSize + '\'>' + chat.created + '</span>';
            if (chat.permissionLevel == 1 && coreSettings == 1) {
              content += '<img src= /img/close_b.png alt=履歴削除  width=21 height=21 onclick = openChatDeleteDialog(' +
                  chat.id + ',' + chat.t_histories_id + ',"' + forDeletionMessage + '","' + created +
            ...
            || type === chatApi.messageType.scenario.message.buttonUI
            || type === chatApi.messageType.scenario.message.button
            || type === chatApi.messageType.scenario.message.pulldown) {
            ...
            } else if (type === chatApi.messageType.scenario.message.checkbox) {
              for(var i=0; i < messageContent.settings.options.length; i++) {
                if(i === (messageContent.settings.options.length - 1)) {
                  textOfMessage += '■ ' + messageContent.settings.options[i];
                } else {
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>シナリオメッセージ(選択肢)(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
...
//        }
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>シナリオメッセージ(ファイル送信)' + (isExpired ? '（ダウンロード有効期限切れ）' : '') + '</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important; font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
            ...
            message = JSON.parse(message);
            var forDeletionMessage = message.fileName.replace(/\r?\n?\s+/g, '');
            content = '<span class=\'cName\' style = \'font-size:' + fontSize + '\'>シナリオメッセージ(ファイル送信)' +
                (isExpired ? '（ダウンロード有効期限切れ）' : '') + '</span>';
            content += '<span class=\'cTime\' style = \'font-size:' + timeFontSize + '\'>' + chat.created + '</span>';
            if (chat.permissionLevel == 1 && coreSettings == 1) {
              content += '<img src= /img/close_b.png alt=履歴削除 width=21 height=21 onclick = openChatDeleteDialog(' +
          ...
          if (chat.delete_flg != 1 && !isJSON(message)) {
            ...
            content += $scope.createTextOfRecieveFile(chat, message.downloadUrl, message.fileName, message.fileSize,
                message.extension, message.comment);
          }
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>シナリオメッセージ(一括ヒアリング回答)(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>シナリオメッセージ(一括ヒアリング解析結果)(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
            ...
            content += '<span class=\'cChat\' style = \'font-size:' + fontSize + '\'>' +
                $scope.createBulkHearingAnalyseData(chat, message) + '</span>';
          }
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>シナリオメッセージ(一括ヒアリング内容修正)(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
            ...
            content += '<span class=\'cChat\' style = \'font-size:' + fontSize + '\'>' +
                $scope.createBulkHearingKeyValue(chat, message) + '</span>';
          }
        } else if (type === chatApi.messageType.scenario.message.pulldown) {
          cn = 'sinclo_auto';
          var created = chat.created.replace(' ', '%');
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>シナリオメッセージ（ヒアリング：プルダウン）(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
          } else {
            content = '<span class=\'cName\' style = \'font-size:' + fontSize + '\'>シナリオメッセージ（ヒアリング：プルダウン）</span>';
            content += '<span class=\'cTime\' style = \'font-size:' + timeFontSize + '\'>' + chat.created + '</span>';
            if (chat.permissionLevel == 1 && coreSettings == 1) {
              content += '<img src= /img/close_b.png alt=履歴削除  width=21 height=21 onclick = openChatDeleteDialog(' +
                  chat.id + ',' + chat.t_histories_id + ',"' + forDeletionMessage + '","' + created +
            ...
            textOfMessage = messageObj.message + "\n";
            for(var i=0; i < messageObj.settings.options.length; i++) {
              if(i === (messageObj.settings.options.length - 1)) {
                textOfMessage += '■ ' + messageObj.settings.options[i];
              } else {
                textOfMessage += '■ ' + messageObj.settings.options[i] + "\n";
              }
            }
          ...
          var btnTypeLabel = messageObj.type === 1 ? 'ラジオボタン' : 'ボタン';
          forDeletionMessage = escape_html(forDeletionMessage);
          div.style.textAlign = 'right';
          div.style.height = 'auto';
          div.style.padding = '0';
          ...
          if (chat.delete_flg == 1) {
            var deleteUser = userList[Number(chat.deleted_user_id)];
            content = '<span class=\'cName\' style = \'color:#bdbdbd !important; font-size:' + fontSize +
                '\'>チャットツリーメッセージ（分岐：' + btnTypeLabel + '）(' + Number($('#visitorsId').text()) + ')</span>';
            content += '<span class=\'cTime\' style = \'color:#bdbdbd !important;font-size:' + timeFontSize + '\'>' +
                chat.created + '</span>';
            content += '<span class=\'cChat\' style = \'color:#bdbdbd; font-size:' + fontSize + '\'>(このメッセージは' +
                chat.deleted + 'に' + deleteUser + 'さんによって削除されました。)</span>';
          } else {
            content = '<span class=\'cName\' style = \'font-size:' + fontSize + '\'>チャットツリーメッセージ（分岐：' + btnTypeLabel + '）</span>';
            content += '<span class=\'cTime\' style = \'font-size:' + timeFontSize + '\'>' + chat.created + '</span>';
            if (chat.permissionLevel == 1 && coreSettings == 1) {
              content += '<img src= /img/close_b.png alt=履歴削除  width=21 height=21 onclick = openChatDeleteDialog(' +
                  chat.id + ',' + chat.t_histories_id + ',"' + forDeletionMessage + '","' + created +
            ...
            textOfMessage = messageObj.message + "\n";
            for(var i=0; i < Object.keys(messageObj.labels).length; i++) {
              if(i === (Object.keys(messageObj.labels).length - 1)) {
                textOfMessage += (messageObj.labels[i].type === '1' ? '[] ' : '') + messageObj.labels[i].value;
              } else {
                textOfMessage += (messageObj.labels[i].type === '1' ? '[] ' : '') + messageObj.labels[i].value + "\n";
              }
            }
          ...
          if (type === chatApi.messageType.start) {
            content = '<span style = \'color:#9bbb59 !important; font-size:' + fontSize + '\'>－　' + userName +
                'が入室しました　－</span>';
            content += '<span class=\'cTime\' style = \'font-size:' + timeFontSize + '\'>' + chat.created + '</span>';
          }
        ...
        if (type == chatApi.messageType.scenario.customer.sendFile && $('.recieveFileContent').length !== 0) {
          ...
          $('.recieveFileContent')[$('.recieveFileContent').length - 1].addEventListener('click', function(event) {
      ...
      $scope.createBulkHearingAnalyseData = function(chat, data) {
        ...
        try {
          ...
          Object.keys(obj.target).forEach(function(elm, idx, arr) {
            resultVal += obj.target[elm].label + '：' +
                (obj.message[obj.target[elm].inputType] ? obj.message[obj.target[elm].inputType] : '（なし）') + '\n';
          });
        } catch (e) {
      ...
      $scope.createTextOfSendFile = function(chat, url, name, size, extension, isExpired) {
        ...
        if (extension.match(/(jpeg|jpg|gif|png)$/i) != null && !isExpired) {
          ...
          thumbnail = '<i class=\'fa ' + selectFontIconClassFromExtension(extension) +
      ...
      $scope.createTextOfRecieveFile = function(chat, url, name, size, extension, comment) {
        ...
        if (extension.match(/(jpeg|jpg|gif|png)$/i) != null) {
          ...
          thumbnail = '<i class=\'fal ' + selectFontIconClassFromExtension(extension) +
              ' fa-4x recieveFileThumbnail\' aria-hidden=\'true\'></i>';
      ...
      $scope.ui = function(ip, id) {
        ...
        if (mCustomerInfoList.hasOwnProperty(id) && mCustomerInfoList[id] !== '' && mCustomerInfoList[id] != null &&
            mCustomerInfoList[id] !== undefined) {
      ...
      angular.element('label[for="g_chat"]').on('change', function(e) {
        loading.load.start();
        var url = "<?=$this->Html->url(['controller' => 'ChatHistories', 'action' => 'index'])?>?isChat=" +
            e.target.checked;
        location.href = url;
      });
      <?php endif; ?>
      ...
      /* パラメーターを取り除く */
      var targetParams = <?php echo json_encode(array_flip($excludeList['params']),
        JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);?>;
  ...
  $(document).ready(function() {
    ...
    outputCSVBtn.addEventListener('click', function() {
      ...
      document.getElementById('HistoryIndexForm').action = '<?=$this->Html->url([
        "controller" => "ChatHistories",
        "action" => "outputCSVOfHistory"
      ])?>';
    ...
    $('.cancelBtn').on('click', function() {
      $('#mainDatePeriod').
          html(historySearchConditions.History.period + ' : ' + historySearchConditions.History.start_day + '-' +
              historySearchConditions.History.finish_day);
    });
    ...
    $(window).on('keydown', function(e) {
      ...
      if (e.keyCode === 40 && focusText.className !== 'infoData') { // ↓
        ...
        if (prevBoldTarget.next('tr')[0] != null && parseInt($('.dataTables_scrollBody').css('height')) -
            (focusHeigt - ($('.dataTables_scroll').offset()['top'] + 49) +
                prevBoldTarget.next('tr')[0]['clientHeight']) < 0) {
          $('.dataTables_scrollBody').
              scrollTop($('.dataTables_scrollBody').scrollTop() + prevBoldTarget.next('tr')[0]['clientHeight']);
          ...
          $('.dataTables_scrollBody').
              scrollTop($('.dataTables_scrollBody').scrollTop() + prevBoldTarget[0]['clientHeight']);
        }
      }
      if (e.keyCode === 38 && focusText.className !== 'infoData') { // ↑キーを押したら
        ...
        var $scope = angular.element(element).scope();
        $scope.getOldChat(id, false);
        if (prevBoldTarget.prev('tr')[0] != null && focusHeigt - ($('.dataTables_scroll').offset()['top'] + 49) -
            prevBoldTarget.prev('tr')[0]['clientHeight'] < 0) {
          $('.dataTables_scrollBody').
              scrollTop($('.dataTables_scrollBody').scrollTop() - prevBoldTarget.prev('tr')[0]['clientHeight']);
