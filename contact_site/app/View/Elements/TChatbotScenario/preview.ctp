...
<section ng-repeat="(setActionId, setItem) in setActionList" id="action{{setActionId}}_preview">
  ...
  <ul class="chatTalk details">
    ...
    <div ng-repeat="(index, hearings) in setItem.hearings">
      ...
      <li ng-show="hearings.errorMessage" class="sinclo_re chat_left details" ng-class="{notNone: widget.re_border_none === '' || widget.re_border_none === false, boxType: widget.settings['chat_message_design_type'] == 1, balloonType: widget.settings['chat_message_design_type'] == 2, middleSize: widget.settings['widget_size_type'] == 2, largeSize: widget.settings['widget_size_type'] == 3 || widget.settings['widget_size_type'] == 4}"><span ng-if="widget.settings['show_automessage_name'] === '1'" class="cName details">{{widget.settings['sub_title']}}</span><span id="action{{setActionId}}_error_message" class="details">{{hearings.errorMessage}}</span></li>
    </div>
    <!-- 確認メッセージ -->
    <div>
