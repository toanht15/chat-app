...
#tchatbotscenario_form_preview_body .chatTalk li select { border: 1px solid #909090; border-radius: 4px; padding: 3px 0 5px; margin-top: 10px; max-width: 200px; display: block; margin-bottom: -32px }
  ...
  <ul class="chatTalk details">
    ...
    <div ng-repeat="(index, hearings) in setItem.hearings">
      ...
      <li ng-if="hearings.uiType === '5'" class="sinclo_re chat_left details" ng-class="{notNone: widget.re_border_none === '' || widget.re_border_none === false, boxType: widget.settings['chat_message_design_type'] == 1, balloonType: widget.settings['chat_message_design_type'] == 2, middleSize: widget.settings['widget_size_type'] == 2, largeSize: widget.settings['widget_size_type'] == 3}"><span ng-if="widget.settings['show_automessage_name'] === '1'" class="cName details">{{widget.settings['sub_title']}}</span><span class="details">{{hearings.message}} <div id="action{{setActionId}}_calendar{{index}}"></div></span></li>
        <li ng-show="hearings.message || hearings.options[hearings.uiType][0]" ng-if="hearings.uiType === '4'" class="sinclo_re chat_left details" ng-class="{notNone: widget.re_border_none === '' || widget.re_border_none === false, boxType: widget.settings['chat_message_design_type'] == 1, balloonType: widget.settings['chat_message_design_type'] == 2, middleSize: widget.settings['widget_size_type'] == 2, largeSize: widget.settings['widget_size_type'] == 3}"><span ng-if="widget.settings['show_automessage_name'] === '1'" class="cName details">{{widget.settings['sub_title']}}</span><div><span class="sinclo-text-line">{{hearings.message}}</span><br><select id="action{{setActionId}}_selection{{index}}"><option class="action{{setActionId}}_selection{{index}}_option" value="">選択してください</option><option class="action{{setActionId}}_selection{{index}}_option" ng-repeat="item in hearings.options[hearings.uiType] track by $index" value="{{item}}" ng-bind="item""></option></select></div>
      </li>
