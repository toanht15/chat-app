  ...
  <ul class="chatTalk details">
    ...
    <div ng-repeat="(index, hearings) in setItem.hearings">
      <style ng-if="hearings.uiType === '5'">
        ...
        #action{{setActionId}}_calendar{{index}} .flatpickr-calendar .dayContainer .flatpickr-day.selected { background-color: {{hearings.settings.customDesign.headerBackgroundColor}}; border: 1px solid {{hearings.settings.customDesign.headerBackgroundColor}} !important; color: {{hearings.selectedTextColor}} !important;}
        #action{{setActionId}}_calendar{{index}} .flatpickr-calendar .dayContainer .flatpickr-day.today { border: 1px solid  {{hearings.settings.customDesign.headerBackgroundColor}} !important;}
        #action{{setActionId}}_calendar{{index}} .flatpickr-calendar .dayContainer .flatpickr-day { border-top: 1px solid  {{hearings.settings.customDesign.headerWeekdayBackgroundColor}}; border-right: 1px solid  {{hearings.settings.customDesign.headerWeekdayBackgroundColor}};}
      ...
      <li ng-if="hearings.uiType === '5'" class="sinclo_re chat_left details" ng-class="{notNone: widget.re_border_none === '' || widget.re_border_none === false, boxType: widget.settings['chat_message_design_type'] == 1, balloonType: widget.settings['chat_message_design_type'] == 2, middleSize: widget.settings['widget_size_type'] == 2, largeSize: widget.settings['widget_size_type'] == 3}"><span ng-if="widget.settings['show_automessage_name'] === '1'" class="cName details">{{widget.settings['sub_title']}}</span><span class="details">{{hearings.message}} <div id="action{{setActionId}}_calendar{{index}}" style="margin-top: 7px; margin-bottom: 6px"></div></span></li>
        <li ng-show="hearings.message || hearings.settings.options[0]" ng-if="hearings.uiType === '4'" class="sinclo_re chat_left details" ng-class="{notNone: widget.re_border_none === '' || widget.re_border_none === false, boxType: widget.settings['chat_message_design_type'] == 1, balloonType: widget.settings['chat_message_design_type'] == 2, middleSize: widget.settings['widget_size_type'] == 2, largeSize: widget.settings['widget_size_type'] == 3 || widget.settings['widget_size_type'] == 4}" style="padding-bottom: 0"><span ng-if="widget.settings['show_automessage_name'] === '1'" class="cName details">{{widget.settings['sub_title']}}</span><span class="sinclo-text-line">{{hearings.message}}</span><br><select id="action{{setActionId}}_selection{{index}}"><option class="action{{setActionId}}_selection{{index}}_option" value="">選択してください</option><option class="action{{setActionId}}_selection{{index}}_option" ng-repeat="item in hearings.settings.options track by $index" value="{{item}}" ng-bind="item""></option></select>
      </li>
