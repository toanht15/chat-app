<div ng-if="setItem.actionType == <?= C_SCENARIO_ACTION_LEAD_REGISTER ?>" class="set_action_item_body action_lead_register" ng-init="main.controllLeadRegister(setActionId)">
  <ul>
    <li class="styleFlexbox">
      ...
      <span>
        <label ng-repeat="(key, item) in makeLeadTypeList" class="pointer"><input type="radio" style="outline:0 ;" ng-init="setItem.makeLeadTypeList = <?= C_SCENARIO_LEAD_REGIST ?>" ng-model="setItem.makeLeadTypeList" ng-value="key" ng-change="main.controllLeadRegister(setActionId)">{{item}}</label>
      </span>
      </div>
    </li>
    <li class="styleFlexbox">
    ...
    <!--既存リスト使用の場合-->
    <li class="styleFlexbox" ng-if="setItem.makeLeadTypeList == <?= C_SCENARIO_LEAD_USE ?> && (setItem.leadId === '1' || setItem.leadId === '2')" >
      <div class="fb15em" style="white-space: nowrap; padding-top: 5px;"><label class="require">登録内容</label><span class="questionBalloon"><icon class="questionBtn" data-tooltip="リードリストに登録する項目に対して、それぞれ変数を設定できます">?</icon></span></div>
      <div>
        <div class="grid-container-header">
          <div class='area-name' style="text-align: center;"><label class="require">リードリストの項目</label><span class="questionBalloon"><icon class="questionBtn" data-tooltip="変数名で指定された変数の値が、ここで指定されたリードリストの項目に自動で登録されます。">?</icon></span></div>
        ...
        <!-- モック用なのでよくわからない、後でちゃんとng-repeatを理解する必要がある -->
        <div ng-if="setItem.leadId === '1'" class="grid-container-body itemListGroup" ng-repeat="(key, name) in main.leadInfo1 track by $index">
          <div class='area-name'><input type="text" class="make-box" disabled value="{{name.info}}"></div>
          <div class='area-name'><input type="text" class="make-box" value="{{name.var}}"></div>
        </div>
        <div ng-if="setItem.leadId === '2'" class="grid-container-body itemListGroup" ng-repeat="(key, name) in main.leadInfo2 track by $index">
          <div class='area-name'><input type="text" class="make-box" disabled value="{{name.info}}"></div>
          <div class='area-name'><input type="text" class="make-box" value="{{name.var}}"></div>
        </div>
          <hr class="separator short" ng-if="!$last"/>
