  ...
  <ul>
    <li>
      <div class='grid-container grid-container-header'>
        <div class='area-name'>変数名<span class="questionBalloon"><icon class="questionBtn" data-tooltip="変数名を設定します。<br>ここで設定した変数名にサイト訪問者の回答内容が保存されます。<br>変数に保存された値（内容）は後続の処理（アクション）で、<code ng-non-bindable>{{変数名}}</code>と指定することで利用することが可能です。<br><br>例）変数名：名前　⇒　<code ng-non-bindable>{{名前}}</code>様からのお問い合わせを受付いたしました。" data-tooltip-width='300'>?</icon></span></div>
    ...
    <li>
      ...
      <ul ng-if="setItem.isConfirm == true" class="indentDown">
        <li class="styleFlexbox">
          <span class="fb9em"><label>確認内容<span class="questionBalloon"><icon class="questionBtn" data-tooltip="確認メッセージとして送信するメッセージを設定します。<br><br>＜設定例＞<br>お名前　　　　：<code ng-non-bindable>{{名前}}</code><br>電話番号　　　：<code ng-non-bindable>{{電話番号}}</code><br>メールアドレス：<code ng-non-bindable>{{メールアドレス}}</code><br>でよろしいでしょうか？" data-tooltip-width='300'>?</icon></span></label></span>
          <div>
