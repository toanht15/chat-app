...
<div ng-if="setItem.actionType == <?= C_SCENARIO_ACTION_EXTERNAL_API ?>" class="set_action_item_body action_external_api_connection" ng-init="main.controllExternalApiSetting(setActionId)">
  <ul>
    ...
    <li class="styleFlexbox" ng-if="setItem.externalType == <?= C_SCENARIO_EXTERNAL_TYPE_API ?>">
      ...
      <div>
        <input ng-model="setItem.url" type="text" class="variable-suggest system-variable-suggest" placeholder="外部連携先のURLを入力してください">
      </div>
    </li>
    <li class="styleFlexbox" ng-if="setItem.externalType == <?= C_SCENARIO_EXTERNAL_TYPE_API ?>">
      ...
      <span>
        <label ng-repeat="(key, item) in apiMethodType" class="pointer"><input type="radio" class="variable-suggest system-variable-suggest" ng-model="setItem.methodType" ng-value="key">{{item}}</label>
      </span>
    </li>
    <li class="styleFlexbox" ng-if="setItem.externalType == <?= C_SCENARIO_EXTERNAL_TYPE_API ?>">
      ...
      <div>
        <table cellspacing="5">
          ...
          <tr ng-repeat="(listId, headerItem) in setItem.requestHeaders">
            <td class="item_name"><input ng-model="headerItem.name" class="variable-suggest system-variable-suggest" type="text" class="frame"></td>
            <td class="item_value"><input ng-model="headerItem.value" class="variable-suggest system-variable-suggest" type="text" class="frame"></td>
    ...
    <li class="styleFlexbox" ng-if="setItem.methodType == <?= C_SCENARIO_METHOD_TYPE_POST ?> && setItem.externalType == <?= C_SCENARIO_EXTERNAL_TYPE_API ?>">
      ...
      <div>
        <resize-textarea ng-model="setItem.requestBody" class="variable-suggest system-variable-suggest" cols="48" rows="1" placeholder="リクエストボディを設定してください" data-maxRow="10"></resize-textarea>
      </div>
    </li>
    <li class="styleFlexbox" ng-if="setItem.externalType == <?= C_SCENARIO_EXTERNAL_TYPE_API ?>">
      ...
      <span>
        <label ng-repeat="(key, item) in apiResponseType" class="pointer"><input class="variable-suggest system-variable-suggest" type="radio" ng-model="setItem.responseType" ng-value="key">{{item}}</label>
      </span>
    </li>
    <li class="styleFlexbox" ng-if="setItem.externalType == <?= C_SCENARIO_EXTERNAL_TYPE_API ?>">
      ...
      <div>
        <table cellspacing="5">
          ...
          <tr ng-repeat="(listId, bodyItem) in setItem.responseBodyMaps">
            ...
            <td class="item_value"><input ng-model="bodyItem.sourceKey" type="text" class="variable-suggest system-variable-suggest frame"></td>
            <td class="item_btn_block">
              <div class="btnBlock">
                <a><?= $this->Html->image('add.png', array('alt' => '追加', 'width' => 25, 'height' => 25, 'class' => 'btn-shadow disOffgreenBtn', 'style' => 'padding: 2px', 'ng-click' => 'main.addActionItemList($event, listId)')) ?></a><a><?= $this->Html->image('dustbox.png', array('alt' => '削除', 'width' => 25, 'height' => 25, 'class' => 'btn-shadow redBtn deleteBtn', 'style' => 'padding: 2px', 'ng-click' => 'main.removeActionItemList($event, listId)')) ?></a>
              </div>
  ...
  document.body.appendChild(img);" ng-required="true" required="required" class="variable-suggest system-variable-suggest ng-invalid ng-invalid-required ng-valid-maxlength ng-touched"></textarea>
      </div>
    </li>
    <li class="styleFlexbox align_center" ng-if="setItem.externalType == <?= C_SCENARIO_EXTERNAL_TYPE_SCRIPT ?>">
      <i class="icon fal fa-lightbulb-on" style="margin:0 5px 0 156px; font-size:16px;"></i><a href="https://info.sinclo.jp/manual/3213/" target="_blank" id = "help_link">Google／Yahoo広告のコンバージョンをトラッキングする設定方法</a>
    </li>
  </ul>
</div>
