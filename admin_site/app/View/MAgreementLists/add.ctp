...
function saveFile(){
  location.href = "<?=$this->Html->url(array('controller' => 'MAgreementLists', 'action' => 'add'))?>";
}

//パスワード自動生成
function saveAct(){
  var key = $('#MAgreementListCompanyKey').val();
  $('#MAgreementListMailAddress').val(key+"@ml.jp");
  $("#MAgreementListMailAddress").prop("disabled", false);
  if ($('#MAgreementListTrialFlg').prop('checked')) {
    var day = $('MAgreementListUseStart').val();
    var d = new Date($('#MAgreementListUseStart').val());
    var endDate = d.getFullYear() + '/' + (d.getMonth() + 1) + '/' + (d.getDate() + 14);
    $('#MAgreementListUseEnd').val(endDate);
    console.log($('#MAgreementListUseEnd').val());
  }
  else {
    var day = $('MAgreementListUseStart').val();
    var d = new Date($('#MAgreementListUseStart').val());
  ...
  document.getElementById('MAgreementListAddForm').submit();
}
...
$(function(){
  $("#MAgreementListMailAddress").prop("disabled", true);
});
</script>
