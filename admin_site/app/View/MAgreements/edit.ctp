...
//パスワード自動生成
function saveAct(){
  if ($('#MAgreementTestUse').prop('checked')) {
    var day = $('MAgreementagreement_start_day').val();
    var d = new Date($('#MAgreementagreement_start_day').val());
    var endDate = d.getFullYear() + '/' + (d.getMonth() + 1) + '/' + (d.getDate() + 14);
    $('#MAgreementagreement_end_day').val(endDate);
    console.log($('#MAgreementagreement_end_day').val());
  }
  else {
    var day = $('MAgreementagreement_start_day').val();
    var d = new Date($('#MAgreementagreement_start_day').val());
