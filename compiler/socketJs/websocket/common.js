    ...
    widgetCssTemplate: function(widget){
      ...
      */
      ...
      if ( window.sincloInfo.contract.chat ) {
        ...
        html += '      #sincloBox ul#chatTalk li sinclo-radio [type="radio"] + label:before { content: ""; vertical-align: middle; display: flex; align-self: center; width: ' + (Number(widget.reTextSize)+1) + 'px; height: ' + (Number(widget.reTextSize)+1) + 'px; border: 0.5px solid #999; border-radius: 50%; background-color: #FFF; } ';
        html += '      #sincloBox ul#chatTalk li sinclo-radio [type="radio"]:checked + label:after { content: ""; display: flex; align-self: center; position: absolute; left: ' + ((widget.reTextSize/2 - ((widget.reTextSize-6)/2))+1) + 'px; width: ' + (Number(widget.reTextSize)-6) + 'px; height: ' + (Number(widget.reTextSize)-6) + 'px; background: ' + colorList['mainColor'] + '; border-radius: 50%; } ';
