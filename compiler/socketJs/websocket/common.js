...
(function(jquery) {
  ...
  common = {
    ...
    fullDateTime: function(parse) {
      ...
      return d.getFullYear() + '-' + _numPad(d.getMonth() + 1) + '-' +
          _numPad(d.getDate()) + ' ' + _numPad(d.getHours()) + ':' +
          _numPad(d.getMinutes()) + ':' + _numPad(d.getSeconds()) + '.' +
          Number(String(d.getMilliseconds()).slice(0, 2));
    },
    ...
    createWidget: function() {
      // 右下か左下に固定
      if(check.hasCustomBannerImageSetting() && check.smartphone()) {
        if (Number(window.sincloInfo.widget.spBannerPosition) === 3) {
          // 右中央 => 右下
          window.sincloInfo.widget.spBannerPosition = 1;
        } else if (Number(window.sincloInfo.widget.spBannerPosition) === 4) {
          // 右中央 => 右下
          window.sincloInfo.widget.spBannerPosition = 2;
        }
      }
      ...
      else if (window.sincloInfo.contract.chat &&
          (window.sincloInfo.contract.synclo ||
              (window.sincloInfo.contract.hasOwnProperty('document') &&
      ...
      if (Number(widget.closeButtonSetting) === 2 &&
        ...
        return '<sinclo id=\'sincloBox\' >' + css +
            '<div id=\'sincloWidgetBox\'>' +
            header + displaySet + fotter +
            '</div><div id=\'sincloBannerBox\' style=\'display: none;\'>' +
            sincloBanner + '</div></sinclo>';
      } else {
        //今までと同じ通常のsinclo
        return '<sinclo id=\'sincloBox\' >' + css +
            '<div id=\'sincloWidgetBox\'>' +
    ...
    getColorList: function(widget) {
      ...
      if (Number(widget.colorSettingType) === 0) {
        ...
        //高度な設定
        if (widget.reBorderColor === undefined || widget.reBorderColor ===
            'none') {
        ...
        if (widget.seBorderColor === undefined || widget.seBorderColor ===
            'none') {
        ...
        if (widget.messageBoxBorderColor === undefined ||
            widget.messageBoxBorderColor === 'none') {
        ...
        if (widget.widgetBorderColor === undefined ||
            widget.widgetBorderColor === 'none') {
    ...
    _headerContentsSettings: {
      _processSettings: function() {
        ...
        this._deciedPosition('#widgetTitle',
            Number(sincloInfo.widget.widget_title_top_type));
        this._deciedPosition('#widgetSubTitle',
            Number(sincloInfo.widget.widget_title_name_type));
        this._deciedPosition('#widgetDescription',
            Number(sincloInfo.widget.widget_title_explain_type));
      ...
      _countContents: function() {
        if (Number(sincloInfo.widget.showDescription) === 1 &&
            Number(sincloInfo.widget.showSubtitle) === 1) {
          return 2;
        } else if (Number(sincloInfo.widget.showDescription) === 1 ||
            Number(sincloInfo.widget.showSubtitle) === 1) {
    ...
    abridgementTypeShow: function() {
      ...
      if ((Number(window.sincloInfo.widget.showMainImage) === 2 ||
          window.sincloInfo.widget.mainImage === '')) {
    ...
    isPortrait: function() {
      ...
      return (o && common.defaultOrientation) ||
          !(o || common.defaultOrientation);
    },
    widgetCssTemplate: function(widget) {
      ...
      var html = '', faintColor = widget.mainColor,
          balloonInnerColor = faintColor = widget.mainColor,
      ...
      if (faintColor.indexOf('#') >= 0) {
        ...
        faintColor = 'rgba(' + parseInt(r, 16) + ', ' + parseInt(g, 16) + ', ' +
            parseInt(b, 16) + ', 0.1)';
        var balloonR = String(Math.floor(255 - (255 - parseInt(r, 16)) * 0.1));
        var balloonG = String(Math.floor(255 - (255 - parseInt(g, 16)) * 0.1));
        var balloonB = String(Math.floor(255 - (255 - parseInt(b, 16)) * 0.1));
        balloonInnerColor = 'rgb(' + balloonR + ', ' + balloonG + ', ' +
            balloonB + ');';
        var highlightColorR = String(
            Math.floor(255 - (255 - parseInt(r, 16)) * 0.7));
        var highlightColorG = String(
            Math.floor(255 - (255 - parseInt(g, 16)) * 0.7));
        var highlightColorB = String(
            Math.floor(255 - (255 - parseInt(b, 16)) * 0.7));
        highlightColor = 'rgb(' + highlightColorR + ', ' + highlightColorG +
            ', ' + highlightColorB + ');';
      }
      ...
      switch (Number(widget.showPosition)) {
        case 1: // 右下
          showPosition = 'bottom: ' + widgetVerticalPosition + '; right: ' +
          ...
          break;
        case 2: // 左下
          showPosition = 'bottom: ' + widgetVerticalPosition + '; left: ' +
              widgetHorizontalPosition + ';';
          chatPosition = {
            se: {
              mg: 'margin-left: 10px;',
      ...
      //アイコンフォント用
      html += '      @font-face { font-family: "FA5P"; font-style: normal; font-weight: 300; src: url("' +
          sincloInfo.site.files + '/webfonts/fa-light-300.eot"); src: url("' +
          ...
          sincloInfo.site.files +
          '/webfonts/fa-light-300.svg#fontawesome") format("svg"); }';
      html += '      @font-face { font-family: SincloFont; font-style: normal; font-weight: 900; src: url("' +
          ...
          sincloInfo.site.files +
          '/webfonts/fa-solid-900.ttf") format("truetype"), url("' +
          sincloInfo.site.files +
      ...
      */
      ...
      //END OF reset-css
      html += '      #sincloBox { display: none; position: fixed; ' +
          showPosition + ' z-index: 999998; background-color: rgba(0,0,0,0); }';
      ...
      html += '      #sincloBox .sinclo_se a { color: ' +
          chatPosition.se.color + ' }';
      html += '      #sincloBox .sinclo_re a { color: ' +
          chatPosition.re.color + ' }';
      html += '      #sincloBox ul#chatTalk li.sinclo_re span.telno { color: ' +
          chatPosition.re.color + '; }';
      ...
      html += '      #sincloBox span#mainImage img { background-color: ' +
          colorList['mainColor'] + ' }';
      html += '      #sincloBox span#mainImage i.normal { color: ' +
      ...
      if (Number(widget.closeButtonSetting) === 1) {
        ...
        html += '      #sincloBox div#closeBtn { display: none; cursor: pointer; background-image: url("' +
            window.sincloInfo.site.files +
            '/img/widget/close.png"); background-position-y: -1.5px; position: absolute; top: calc(50% - 9px); right: 6px; content: " "; width: 18px; height: 18px; background-size: contain; vertical-align: middle; background-repeat: no-repeat; transition: transform 200ms linear; z-index: 2; }';
      } else {
        //閉じるボタン有効
        html += '      #sincloBox div#closeBtn { display: none; cursor: pointer; background-image: url("' +
            window.sincloInfo.site.files +
      ...
      if (window.sincloInfo.contract.chat) {
        ...
        // ------------ファイルフォントアイコン
        html += '      #sincloBox ul#chatTalk li span.sendFileMessage { color: ' +
            chatPosition.re.color + ' }';
        ...
        html += '      #sincloBox ul#chatTalk li div.sendFileContent * { color: ' +
            chatPosition.re.color + ' }';
        ...
        html += '      #sincloBox ul sinclo-typing { padding-bottom: ' +
            sizeList['paddingBottom'] + 'px; display: block; }';
        html += '      #sincloBox ul#chatTalk li { text-align: left!important; word-wrap: break-word; word-break: break-all; white-space: pre-wrap!important; background-color: transparent; white-space: pre; color: ' +
            chatContentTextColor + '; font-weight: normal!important; }';
        html += '      #sincloBox ul#chatTalk li.sinclo_se span.sinclo-text-line { display: inline-block; color:' +
            chatPosition.se.color + '; font-size: ' + chatPosition.se.textSize +
            'px; }';
        html += '      #sincloBox ul#chatTalk li.sinclo_re span.sinclo-text-line { display: inline-block; color:' +
            chatPosition.re.color + '; font-size: ' + chatPosition.re.textSize +
            'px; }';
        if (widget.chatMessageDesignType === 2) {
          ...
          html += '      #sincloBox ul#chatTalk li.' + rightMessageClass +
              ' { ' + chatPosition.se.mg + 'background-color: ' +
              chatPosition.se.backgroundColor +
              '; display: inline-block; position: relative; margin-right: 15px; border-bottom-right-radius: 0px!important; color:' +
              chatPosition.se.color + '; font-size: ' +
              chatPosition.se.textSize + 'px; }';
          html += '      #sincloBox ul#chatTalk li.' + rightMessageClass +
              ':before { height: 0px; content: ""; position: absolute; bottom: 0px; left: calc(100% - 3px); margin-top: -10px; border: 5px solid transparent; border-left: 5px solid ' +
              chatPosition.se.backgroundColor + '; border-bottom: 5px solid ' +
              chatPosition.se.backgroundColor + '; z-index: 2; }';
          html += '      #sincloBox ul#chatTalk li.' + rightMessageClass +
              ':after { height: 0px; content: ""; position: absolute; bottom: -1px; left: 100%; margin-top: -9px; border: 5px solid transparent; z-index: 1 }';
          if (colorList['seBorderNone'] === 0) {
            html += '      #sincloBox ul#chatTalk li.' + rightMessageClass +
                ':after {border-left: 5px solid ' +
                chatPosition.se.borderColor + '; border-bottom: 5px solid ' +
                chatPosition.se.borderColor + '; }';
          }
          html += '      #sincloBox ul#chatTalk li.' + leftMessageClass +
              ' { ' + chatPosition.re.mg + 'background-color:' +
              chatPosition.re.backgroundColor +
              '; display: inline-block; position: relative; margin-left: 10px; padding-right: 15px; border-bottom-left-radius: 0px!important; color:' +
          ...
          // BOX型
          html += '      #sincloBox ul#chatTalk li.sinclo_se { ' +
              chatPosition.se.mg + 'background-color: ' +
              chatPosition.se.backgroundColor + '; color:' +
              chatPosition.se.color + '; font-size: ' +
              chatPosition.se.textSize +
              'px; display: inline-block; position: relative;}';
          html += '      #sincloBox ul#chatTalk li.sinclo_re { ' +
              chatPosition.re.mg + 'background-color:' +
              chatPosition.re.backgroundColor + '; color:' +
              chatPosition.re.color + '; font-size: ' +
              chatPosition.re.textSize +
              'px; display: inline-block; position: relative;}';
        }
        if (widget.chatMessageWithAnimation === 1) {
          ...
          html += '      #sincloBox ul#chatTalk :not(.grid_for_icon) > li.effect_left { -webkit-animation-name:leftEffect; animation-name:leftEffect; -webkit-animation-duration:0.5s; animation-duration:0.5s; -webkit-animation-iteration-count:1; animation-iteration-count:1; -webkit-animation-fill-mode:both; animation-fill-mode:both; -webkit-transform-origin:left bottom; transform-origin:left bottom; opacity:0; -webkit-animation-delay:0.6s; animation-delay:0.6s; }';
          html += '      #sincloBox ul#chatTalk .grid_for_icon.effect_left { -webkit-animation-name:leftEffect; animation-name:leftEffect; -webkit-animation-duration:0.5s; animation-duration:0.5s; -webkit-animation-iteration-count:1; animation-iteration-count:1; -webkit-animation-fill-mode:both; animation-fill-mode:both; -webkit-transform-origin:left bottom; transform-origin:left bottom; opacity:0; -webkit-animation-delay:0.6s; animation-delay:0.6s; }';
          html += '      #sincloBox ul#chatTalk .grid_for_icon.effect_left_wait { -webkit-animation-name:leftEffect; animation-name:leftEffect; -webkit-animation-duration:0.5s; animation-duration:0.5s; -webkit-animation-iteration-count:1; animation-iteration-count:1; -webkit-animation-fill-mode:both; animation-fill-mode:both; -webkit-transform-origin:left bottom; transform-origin:left bottom; opacity:0; }';
        } else {
          ...
          html += '      #sincloBox ul#chatTalk .grid_for_icon.effect_left { -webkit-animation-name:noneLeftEffect; animation-name:noneLeftEffect; -webkit-animation-duration:1ms; animation-duration:1ms; -webkit-animation-iteration-count:1; animation-iteration-count:1; -webkit-animation-fill-mode:both; animation-fill-mode:both; opacity:0; -webkit-animation-delay:0.7s; animation-delay:0.7s; }';
          html += '      #sincloBox ul#chatTalk .grid_for_icon.effect_left_wait { -webkit-animation-name:noneLeftEffect; animation-name:noneLeftEffect; -webkit-animation-duration:1ms; animation-duration:1ms; -webkit-animation-iteration-count:1; animation-iteration-count:1; -webkit-animation-fill-mode:both; animation-fill-mode:both; opacity:0; }';
        }
        ...
        html += '      #sincloBox ul#chatTalk li span.cName { display: block; color: ' +
            colorList['cNameTextColor'] + '; font-weight: bold; }';
        html += '      #sincloBox ul#chatTalk li sinclo-radio { display: inline-block; margin-top: ' +
            widget.btwButtonMargin + 'px; } ';
        html += '      #sincloBox ul#chatTalk li span.sinclo-text-line + sinclo-radio { margin-top: ' +
            widget.lineButtonMargin + 'px; } ';
        ...
        html += '      #sincloBox ul#chatTalk li sinclo-radio [type="radio"] + label { position: relative; display: inline-block; width: 100%; cursor: pointer; margin: 0; padding: 0 0 0 ' +
            (Number(chatPosition.re.textSize) + 7) + 'px; color:' +
            chatPosition.re.color + '; min-height: 12px; font-size: ' +
            chatPosition.re.textSize + 'px; } ';
        html += '      #sincloBox ul#chatTalk li sinclo-radio [type="radio"] + label:before { content: ""; display: block; position: absolute; top: 0px; left: 0px; ' +
            ' width: ' +
        ...
        html += '      #sincloBox ul#chatTalk li sinclo-radio [type="radio"]:checked + label:after { content: ""; display: block; position: absolute; top: ' +
            ...
            ((chatPosition.re.textSize / 2 -
                ((chatPosition.re.textSize - 6) / 2)) + 1) +
            'px; margin-top: -' +
            (Math.round(chatPosition.re.textSize / 2) - 4) + 'px; width: ' +
        ...
        html += '      #sincloBox section#chatTab sinclo-div:not(#flexBoxWrap) { border-top: 1px solid ' +
        ...
        if (Number(widget.widgetSizeType) == 1) {
          html += uploadFileClasses + '.recv_file_left, ' + uploadFileClasses +
              '.recv_file_right { display: block; padding: 10px!important; line-height: 0; }';
          html += previewFileClasses +
              '.uploaded { padding: 10px!important; line-height: 0; }';
          html += uploadFileClasses +
              ' div.receiveFileContent div.selectFileArea p.drop-area-message { margin: 9px 0 6.5px; line-height: 20px; }';
          html += uploadFileClasses +
              ' div.receiveFileContent div.selectFileArea p.drop-area-button { display: flex; justify-content: center; line-height: 0px; margin: 6.5px 0 9px; }';
        } else if (check.smartphone() || Number(widget.widgetSizeType) == 2) {
          html += uploadFileClasses + '.recv_file_left, ' + uploadFileClasses +
              '.recv_file_right { display: block; padding: 12px!important; line-height: 0; }';
          html += previewFileClasses +
              '.uploaded { padding: 12px!important; line-height: 0; }';
          html += uploadFileClasses +
              '  div.receiveFileContent div.selectFileArea p.drop-area-message { margin: 13px 0 6.5px; line-height: 24px; }';
          html += uploadFileClasses +
              '  div.receiveFileContent div.selectFileArea p.drop-area-button { display: flex; justify-content: center; line-height: 0px; margin: 6.5px 0 13px; }';
        } else if (Number(widget.widgetSizeType) == 3) {
          html += uploadFileClasses + '.recv_file_left, ' + uploadFileClasses +
              '.recv_file_right { display: block; padding: 14px!important; line-height: 0; }';
          html += previewFileClasses +
              '.uploaded { padding: 14px!important; line-height: 0; }';
          html += uploadFileClasses +
              ' div.receiveFileContent div.selectFileArea p.drop-area-message { margin: 13px 0 6.5px; line-height: 24px; }';
          html += uploadFileClasses +
              ' div.receiveFileContent div.selectFileArea p.drop-area-button { display: flex; justify-content: center; line-height: 0px; margin: 6.5px 0 13px; }';
        ...
        html += uploadFileClasses +
            ' div.receiveFileContent { position: relative; line-height: 0; border: 1px dashed ' +
            chatPosition.re.color + '; }';
        html += uploadFileClasses +
            ' div.receiveFileContent div.selectFileArea { line-height: 0; }';
        html += uploadFileClasses +
            ' div.receiveFileContent div.selectFileArea p { margin: 6.5px 0; text-align: center; color:' +
            chatPosition.re.color + '; font-weight: bold; }';
        html += uploadFileClasses +
            ' div.receiveFileContent a.select-file-button { display:inline-block; width:75%; padding: 5px 35px; border-radius: 0; text-decoration: none; cursor: pointer; text-align: center; background-color: ' +
            chatPosition.re.color + '!important; color: ' +
            chatPosition.re.backgroundColor + '; font-weight: normal; }';
        html += uploadFileClasses +
            ' div.receiveFileContent a.select-file-button:hover { opacity: .8; }';
        html += uploadFileClasses +
            ' div.cancelReceiveFileArea { margin-top: 5px; }';
        html += uploadFileClasses +
            ' div.cancelReceiveFileArea a { font-size: ' +
            (chatPosition.re.textSize - 1) +
            'px; cursor: pointer; text-decoration: underline; }';
        html += uploadFileClasses +
            ' div.receiveFileContent div.selectFileArea p.drop-area-icon i { line-height: 1; font-size: 3em; color: ' +
            chatPosition.re.textSize + '; }';
        html += previewFileClasses +
            ' div.receiveFileContent { position: relative; line-height: 0; background-color: #FFF; padding: 5px; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.selectFileArea { line-height: 3px; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.selectFileArea p.preview { text-align: center; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.selectFileArea p.preview img.small { max-width: 165px; max-height: 120px; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.selectFileArea p.preview img.middle { max-width: 215px; max-height: 188px; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.selectFileArea p.preview img.large { max-width: 265px; max-height: 285px; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.selectFileArea p.commentarea { text-align: center; width: 100%; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.selectFileArea p.commentarea textarea { display: block; margin: 0 auto; border-radius: 0px; width: 97%; height: 40px; resize: none; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.selectFileArea p.commentarea textarea:focus { outline: none!important; border-color: ' +
            colorList['chatSendBtnBackgroundColor'] + '!important;}';
        html += previewFileClasses +
            ' div.receiveFileContent div.actionButtonWrap { display: flex; justify-content: space-between; width: 97%; margin: 0 auto; font-size: 0px; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.actionButtonWrap a:hover { opacity: .8; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.actionButtonWrap a.cancel-file-button { display: block; margin-right: 2px; width: 49%; height: auto; padding: 5px 10px; border-radius: 0; text-decoration: none; cursor: pointer; margin: 0 auto; text-align: center; background-color: #7F7F7F!important; color: #FFF; font-weight: normal; word-break: keep-all; }';
        html += previewFileClasses +
            ' div.receiveFileContent div.actionButtonWrap a.send-file-button { display: block; margin-left: 2px; width: 49%; height: auto; padding: 5px 10px; border-radius: 0; text-decoration: none; cursor: pointer; margin: 0 auto; text-align: center; background-color: ' +
            colorList['chatSendBtnBackgroundColor'] + '; color: ' +
            colorList['chatSendBtnTextColor'] +
        ...
        html += '#sincloBox #chatTalk li.sinclo_re div.formContentArea p.formMessage { color: ' +
            chatPosition.re.color + '; }';
        ...
        html += '#sincloBox #chatTalk li.sinclo_re div.formElement input.formInput { padding: 5px; border: 1px solid ' +
            chatPosition.re.backgroundColor + '; }';
        ...
        html += '#sincloBox #chatTalk li.sinclo_re div.formArea p.formOKButtonArea span.formOKButton { width: 100px; height: 30px; cursor: pointer; display: inline-flex; justify-content: center; align-items: center; border-radius: 12px; background-color: ' +
            colorList['chatSendBtnBackgroundColor'] + '; color: ' +
            colorList['chatSendBtnTextColor'] + ';  }';
        ...
        html += '#sincloBox #chatTalk li.sinclo_se div.formElement input.formInput { padding: 5px; border: 1px solid ' +
            chatPosition.se.backgroundColor + '; }';
        ...
        html += '#sincloBox #chatTalk li.sinclo_se div.formArea p.formOKButtonArea span.formOKButton { width: 100px; height: 30px; cursor: pointer; display: inline-flex; justify-content: center; align-items: center; border-radius: 12px; background-color: ' +
            colorList['chatSendBtnBackgroundColor'] + '; color: ' +
            colorList['chatSendBtnTextColor'] + '; opacity:0.38;  }';
        ...
        /* Cogmo */
        html += '#sincloBox #chatTalk li.sinclo_re p.sincloButtonWrap { cursor: pointer; background-color: ' +
            chatPosition.re.color +
            '; text-align: center; padding: 10px; margin: 5px 0px;}';
        html += '#sincloBox #chatTalk li.sinclo_re p.sincloButtonWrap:hover { opacity: 0.8 }';
        html += '#sincloBox #chatTalk li.sinclo_re p.sincloButtonWrap span.sincloButton { color: ' +
            chatPosition.re.backgroundColor + '; font-size: ' +
            chatPosition.re.textSize + 'px;}';
        html += '#sincloBox #chatTalk li.sinclo_re.withButton { line-height: 0; }';

        /* ボタン */
        html += '#sincloBox #chatTalk li.sinclo_re.no-wrap { padding: 10px 0 0 0; }';
        ...
        html += '#sincloBox #chatTalk li.sinclo_re div.sincloHearingButtons span.sincloHearingButton { display: flex; cursor: pointer; justify-content: center; align-items: center; width: 100%; padding: 12px; text-align:center; }';
        ...
        html += '      #sincloBox section#chatTab sinclo-div #sincloChatMessage, #sincloBox section#chatTab sinclo-div #miniSincloChatMessage { display: block; height: 100%; min-height: 100%!important; margin: 0; width: 80%; resize: none; color: ' +
            colorList['messageBoxTextColor'] +
            '!important; background-color: ' +
            colorList['messageBoxBackgroundColor'] + '; font-size: ' +
            widget.messageBoxTextSize + 'px;}';
        html += '      #sincloBox section#chatTab sinclo-div #sincloChatMessage:disabled, #sincloBox section#chatTab sinclo-div #miniSincloChatMessage:disabled { background-color: ' +
            colorList['messageBoxBackgroundColor'] + '; opacity: 1; }';
        html += '      #sincloBox section#chatTab sinclo-div #sincloChatMessage:focus, #sincloBox section#chatTab sinclo-div #miniSincloChatMessage:focus { outline: none; border-color: ' +
            colorList['mainColor'] + '!important }';
        if (colorList['messageBoxBorderNone'] === 0) {
          html += '      #sincloBox section#chatTab sinclo-div #sincloChatMessage, #sincloBox section#chatTab sinclo-div #miniSincloChatMessage { border-right-color: ' +
              colorList['chatSendBtnBackgroundColor'] + '!important; }';
          html += '      #sincloBox section#chatTab sinclo-div #sincloChatMessage:focus, #sincloBox section#chatTab sinclo-div #miniSincloChatMessage:focus { border-color: ' +
              colorList['mainColor'] + '!important }';
        }
        html += '      #sincloBox section#chatTab sinclo-div #sincloChatSendBtn, #sincloBox section#chatTab sinclo-div #miniSincloChatSendBtn { display: flex;justify-content:center; align-items: center; height: 100%; width: 20%; text-decoration: none; border-radius: 0 5px 5px 0; cursor: pointer; margin: 0; text-align: center; background-color: ' +
            colorList['chatSendBtnBackgroundColor'] + '; color: ' +
            colorList['chatSendBtnTextColor'] + '; font-weight: bold;}';
        html += '      #sincloBox section#chatTab sinclo-div #sincloChatSendBtn span, #sincloBox section#chatTab sinclo-div #miniSincloChatSendBtn span {color: ' +
            colorList['chatSendBtnTextColor'] + '; }';
        if ((window.sincloInfo.contract.synclo ||
            (window.sincloInfo.contract.hasOwnProperty('document') &&
                window.sincloInfo.contract.document) ||
            (check.isset(widget.showAccessId) && widget.showAccessId === 1))) {
          if (widget.chatMessageCopy === 1) {
            html += '      #sincloBox section#chatTab #sincloAccessInfo { height: ' +
                sizeList['sincloAccessInfoHeight'] +
                'px; text-align: left; padding-left: 0.5em; padding-top: 5px; padding-bottom: 5px; font-size: 0.9em; border-top: 1px solid ' +
                colorList['widgetInsideBorderColor'] +
      ...
      html += '      #sincloBox section#navigation ul li { position: relative; overflow: hidden; cursor: pointer; color: ' +
          colorList['otherTextColor'] +
          '; text-align: center; display: table-cell; line-height: inherit!important; }';
      html += '      #sincloBox section#navigation ul li.selected { background-color: ' +
          colorList['chatTalkBackgroundColor'] + '; }';
...
*/
      ...
      html += '      #sincloBox section#navigation ul li.selected::before{ color: ' +
          colorList['mainColor'] + '; }';
      //閉じるボタン設定が有効かつバナー表示設定になっているかどうか
      if (Number(widget.closeButtonSetting) === 2 &&
          Number(widget.closeButtonModeType) === 1) {
        ...
        if (check.hasCustomBannerImageSetting()) {
          ...
          html += '      #sincloBox div#sincloBannerBox #sincloBanner.sincloBanner { position: relative; z-index: 1; height: 42px; width : -webkit-fit-content !important; width : -moz-fit-content !important; width : fit-content !important; background-color: ' +
              ...
              'px rgba(0,0,0,0.1); border-radius: ' + widget.radiusRatio +
              'px ' +
              widget.radiusRatio + 'px ' + widget.radiusRatio + 'px ' +
              widget.radiusRatio + 'px; color: ' + colorList['stringColor'] +
              '; margin: auto; filter:alpha(opacity=90); -moz-opacity: 0.9; opacity: 0.9; cursor: pointer; }';
        }
        ...
        html += '      #sincloBox div#sincloBannerBox #sincloBanner.sincloBanner i{ color: ' +
            widget.stringColor + '; }';
        ...
        html += '      #sincloBox div#sincloBannerBox #sincloBanner.sincloBanner .bannertext{ color: ' +
            colorList['stringColor'] +
      ...
      if (check.smartphone()) {
        ...
        if (this.isPortrait()) {
          ...
          html += '#sincloBox div#sincloWidgetBox { box-shadow: 0px 0px ' +
              widget.boxShadow * ratio + 'px ' + widget.boxShadow * ratio +
              'px rgba(0,0,0,0.1); border-radius: ' + widget.radiusRatio *
              ratio + 'px ' + widget.radiusRatio * ratio + 'px 0 0;}';
          html += '#sincloBox * { font-size: ' + (12 * ratio) + 'px; }';
          html += '#sincloBox section { width: ' + widgetWidth + 'px }';
          html += '#sincloBox section#navigation ul { width: ' + widgetWidth +
              'px }';
          html += '#sincloBox span#mainImage { top: ' + (7 * ratio) +
              'px; left: ' + (8 * ratio) + 'px; }';
          ...
          html += '#sincloBox p#widgetTitle { border-radius: ' +
              ...
              'px solid ' + colorList['mainColor'] + '; font-size: ' +
              (14 * ratio) + 'px; padding: ' + (7 * ratio) + 'px ' +
              (30 * ratio) + 'px; height: ' + (32 * ratio) + 'px; }';
          if (widget.widgetSizeType !== 1) {
            html += '#sincloBox p#widgetTitle { overflow: hidden; text-overflow: ellipsis; white-space: nowrap;}';
          }
          html += '      #sincloBox ul#chatTalk div.grid_for_icon .iconDiv.customSize { max-width: ' +
              (42 * ratio) + 'px; min-width: ' + (42 * ratio) + 'px; }';
          html += '      #sincloBox ul#chatTalk div.grid_for_icon .iconDiv.customSize .img_wrapper { width: ' +
              (40 * ratio) + 'px; height: ' + (40 * ratio) + 'px; }';
          html += '      #sincloBox ul#chatTalk div.grid_for_icon .iconDiv.customSize .img_wrapper img { max-width: ' +
              (40 * ratio) + 'px; }';
          html += '      #sincloBox ul#chatTalk div.grid_for_icon .iconDiv.customSize i { height: ' +
              (40 * ratio) + 'px; width: ' + (40 * ratio) + 'px; font-size: ' +
              (26 * ratio) + 'px }';
          html += '#sincloBox p#widgetTitle #sincloChatUnread { width: ' +
              (25 * ratio) + 'px; height: ' + (25 * ratio) + 'px; font-size: ' +
              (13 * ratio) + 'px; border-radius: ' + (15 * ratio) +
              'px; margin: ' + (2.5 * ratio) + 'px ' + (6 * ratio) +
              'px; padding: ' + (3 * ratio) + 'px; }';
//        html += '#sincloBox p#widgetTitle:after { background-position-y: ' + (3 * ratio) + 'px; top: ' + (6 * ratio) + 'px; right: ' + (10 * ratio) + 'px; bottom: ' + (6 * ratio) + 'px; width: ' + (20 * ratio) + 'px; height: ' + (20 * ratio) + 'px; }';
          html += '#sincloBox div#minimizeBtn { display: none; top: ' +
...
*/
            html += '#sincloBox div#closeBtn { display: none; top: ' +
                (7 * ratio) + 'px; right: ' + (10 * ratio) + 'px; bottom: ' +
                (6 * ratio) + 'px; width: ' + (18 * ratio) + 'px; height: ' +
                (18 * ratio) + 'px; z-index: 2; }';
          } else {
            //閉じるボタン有効
            /*
...
*/
          ...
          html += '      #sincloBox p#widgetTitle { border-radius: ' +
              ...
              'px 0 0; font-size: ' + (14 * ratio) + 'px; padding: ' +
              (7 * ratio) + 'px 0px ' + (7 * ratio) + 'px 0px; height: ' +
              (32 * ratio) +
              'px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;}';
          html += '      #sincloBox p#widgetTitle.leftPosition { text-align: left; padding-left: ' +
              (70 * ratio) + 'px;}';
          html += '      #sincloBox p#widgetTitle.leftPosition.noImage { padding-left: ' +
              (15 * ratio) + 'px;}';
          html += '      #sincloBox p#widgetTitle.centerPosition { padding-right: ' +
              (30 * ratio) + 'px; padding-left: calc(2.5em + 38px);}';
          html += '      #sincloBox p#widgetTitle.centerPosition.noImage { padding-left: 0px; padding-right: 0px; }';

          html += '      #sincloBox p#widgetSubTitle { background-color: ' +
              colorList['headerBackgroundColor'] + '; border-color: ' +
              colorList['widgetBorderColor'] + '; font-weight: bold; color: ' +
              colorList['subTitleTextColor'] +
              '; border-style: solid; text-align: left; margin: 0; padding: ' +
              (7 * ratio) + 'px 0; border-width: 0 ' + (1 * ratio) + 'px 0 ' +
              (1 * ratio) + 'px; padding-left: ' + (74 * ratio) +
              'px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;}';
          html += '      #sincloBox p#widgetSubTitle.leftPosition { padding-left: ' +
              (70 * ratio) + 'px;}';
          html += '      #sincloBox p#widgetSubTitle.leftPosition.noImage { padding-left: ' +
              (15 * ratio) + 'px;}';
          html += '      #sincloBox p#widgetSubTitle.centerPosition { text-align: center; padding-right: ' +
              (30 * ratio) + 'px; padding-left: calc(2.5em + 38px * ' + ratio +
              ');}';
          html += '      #sincloBox p#widgetSubTitle.centerPosition.noImage { padding-left: 0px; padding-right: 0px;}';
          html += '      #sincloBox p#widgetSubTitle.oneContent { line-height: calc((1em + 9px)*2); padding-bottom: ' +
              (3.5 * ratio) + 'px; padding-top: ' + (4.5 * ratio) +
              'px; border-bottom: 1px ' + colorList['widgetBorderColor'] +
              ' solid;}';
          if (colorList['widgetInsideBorderNone'] === 1) {
            html += '      #sincloBox p#widgetSubTitle.oneContent{ border:none;}';
          }

          html += '      #sincloBox p#widgetDescription.leftPosition { padding-left: ' +
              (70 * ratio) + 'px;}';
          html += '      #sincloBox p#widgetDescription.leftPosition.noImage { padding-left: ' +
              (15 * ratio) + 'px;}';
          html += '      #sincloBox p#widgetDescription.centerPosition { text-align: center; padding-right: ' +
              (30 * ratio) + 'px; padding-left: calc(2.5em + 38px * ' + ratio +
              ');}';
          html += '      #sincloBox p#widgetDescription.centerPosition.noImage { padding-left: 0px; padding-right: 0px;}';
          html += '      #sincloBox p#widgetDescription.oneContent { line-height: calc((1em + 9px)*2); padding-bottom: ' +
              (3.5 * ratio) + 'px; padding-top: ' + (4.5 * ratio) + 'px;}';
          if (widget.widgetSizeType !== 1) {
          ...
          html += '#sincloBox p#widgetDescription { background-color: ' +
              colorList['headerBackgroundColor'] +
              '; margin: 0; padding-bottom: ' + (7 * ratio) +
              'px; border-width: 0 ' + (1 * ratio) + 'px ' + (1 * ratio) +
              'px ' + (1 * ratio) + 'px; padding-left: ' + (74 * ratio) +
              'px; text-align: left; border-color: ' +
              colorList['widgetBorderColor'] +
              '; border-style: solid; color: ' +
              colorList['descriptionTextColor'] + '; border-bottom-color:' +
              colorList['widgetInsideBorderColor'] + '; }';
          if (widget.widgetSizeType !== 1) {
          ...
          html += '#sincloBox section { border: ' + (1 * ratio) + 'px solid ' +
              colorList['widgetBorderColor'] +
              '; border-top: none; border-bottom: ' + (1 * ratio) +
              'px solid ' + colorList['widgetInsideBorderColor'] + ';}';
          if (colorList['widgetBorderNone'] === 1) {
          ...
          // 画像がセットされている場合のスタイル
          html += '#sincloBox p#widgetTitle.notSimple { padding-left: ' +
              (70 * ratio) + 'px; }';
          // 画像がセットされていない場合のスタイル
          html += '#sincloBox p#widgetTitle.noImage { padding-left: ' +
              (30 * ratio) + 'px; }';
          html += '#sincloBox #mainImage em { top: -' + (10 * ratio) +
              'px; right: -' + (10 * ratio) + 'px; width: ' + (25 * ratio) +
              'px; height: ' + (20 * ratio) + 'px; font-size: ' + (11 * ratio) +
              'px; padding: ' + (1 * ratio) + 'px; }';
          if (widget.spMaximizeSizeType === 2) {
            html += '#sincloBox ul#chatTalk { padding: ' + (5 * ratio) +
                'px; padding-top:0px; height: ' + (194 * ratio) +
                'px; background-color: ' +
                colorList['chatTalkBackgroundColor'] + ' }';
          } else {
            html += '#sincloBox ul#chatTalk { padding: ' + (5 * ratio) +
          ...
          html += '#sincloBox ul#chatTalk li { border-radius: ' + (12 * ratio) +
              'px; margin: ' + (5 * ratio) + 'px 0; padding: ' + (10 * ratio) +
              'px ' + (10 * ratio) + 'px; font-size: ' + (12 * ratio) + 'px; }';
          html += '#sincloBox ul#chatTalk li.sinclo_se { font-size: ' +
              (12 * ratio) + 'px; margin-left: ' + (widgetWidth / 342.5) * 45 +
              'px;}';
          html += '#sincloBox ul#chatTalk li.sinclo_re { font-size: ' +
              (12 * ratio) + 'px; margin-right: ' + (widgetWidth / 342.5) * 21 +
              'px;}';
          html += '#sincloBox ul#chatTalk li.sinclo_se span.sinclo-text-line { font-size: ' +
              (12 * ratio) + 'px; }';
          html += '#sincloBox ul#chatTalk li.sinclo_re span.sinclo-text-line { font-size: ' +
              (12 * ratio) + 'px; }';
          html += '#sincloBox ul#chatTalk li div.sendFileThumbnailArea { display: table-cell; width: ' +
              (64 * ratio) + 'px; height: ' + (64 * ratio) +
              'px; border: 1px solid #D9D9D9; }';
          if (colorList['seBorderNone'] === 0) {
            html += '#sincloBox ul#chatTalk li.sinclo_se { border: ' +
                (1 * ratio) + 'px solid ' + chatPosition.se.borderColor + '; }';
          }
          if (colorList['reBorderNone'] === 0) {
            html += '#sincloBox ul#chatTalk li.sinclo_re { border: ' +
                (1 * ratio) + 'px solid ' + chatPosition.re.borderColor + '; }';
          }
          html += '#sincloBox ul#chatTalk li sinclo-radio { margin-top: ' +
              (Number(widget.btwButtonMargin) * ratio) +
              'px; display: inline-block; } ';
          html += '#sincloBox ul#chatTalk li span.sinclo-text-line + sinclo-radio { margin-top: ' +
              (Number(widget.lineButtonMargin) * ratio) + 'px; } ';
          if (widget.chatMessageDesignType === 2) {
            // 吹き出し型
            html += '#sincloBox ul#chatTalk li { line-height: 1.4; padding: ' +
                (10 * ratio) + 'px ' + (15 * ratio) +
          ...
          html += '#sincloBox ul#chatTalk li sinclo-radio [type="radio"] + label:before { content: ""; display: block; position: absolute; top: ' +
              ((11 * ratio) / 2 - (5 * ratio)) + 'px; margin-top: -' +
              (11 * ratio) / 2 +
          ...
          html += '#sincloBox ul#chatTalk li [id^="sinclo-radio-button"] sinclo-radio [type="radio"] + label:before {top: ' +
              ((11 * ratio) / 2 - (5 * ratio)) + 'px; ' +
              ' left: 0; width: ' + (12 * ratio) + 'px; height: ' + (12 * ratio) + 'px; }';
          html += '#sincloBox ul#chatTalk li [id^="sinclo-radio-button"] sinclo-radio [type="radio"] + label:after {top: ' +
          ...
          if (colorList['seBorderNone'] === 0) {
            html += '      #sincloBox ul#chatTalk li.sinclo_se:after {border-left: 5px solid ' +
                chatPosition.se.borderColor + '; border-bottom: 5px solid ' +
                chatPosition.se.borderColor + '; }';
          }
          if (colorList['reBorderNone'] === 0) {
            html += '      #sincloBox ul#chatTalk li.sinclo_re:after {border-right: 5px solid ' +
                chatPosition.re.borderColor + '; border-bottom: 5px solid ' +
                chatPosition.re.borderColor + '; }';
          }
          html += '#sincloBox ul#chatTalk sinclo-chat-receiver #receiveMessage { font-size: ' +
              (10 * ratio) + 'px; }';
          html += '#sincloBox section#chatTab sinclo-div:not(#flexBoxWrap) { height: ' +
              (60 * ratio) + 'px; padding: ' + (5 * ratio) + 'px; }';
          if (colorList['widgetInsideBorderNone'] === 0) {
            html += '#sincloBox section#chatTab sinclo-div:not(#flexBoxWrap) { border-top: ' +
                (1 * ratio) + 'px solid ' +
                colorList['widgetInsideBorderColor'] + '; }';
          }
          html += '#sincloBox section#chatTab #sincloChatMessage, #sincloBox section#chatTab #miniSincloChatMessage { font-size: ' +
              (13 * ratio) + 'px! important; padding: ' +
              (5 * ratio) + 'px;  }';
          if (colorList['messageBoxBorderNone'] === 0) {
            html += '#sincloBox section#chatTab #sincloChatMessage, #sincloBox section#chatTab #miniSincloChatMessage { border-radius: ' +
                (5 * ratio) + 'px 0 0 ' + (5 * ratio) +
                'px!important; border: ' + (1 * ratio) + 'px solid ' +
                colorList['messageBoxBorderColor'] + '!important; }';
          } else {
          ...
          // 一括ヒアリング
          html += '#sincloBox #chatTalk li.sinclo_re.sinclo_form { padding: ' +
              (10 * ratio) + 'px ' + (15 * ratio) + 'px ' + (15 * ratio) +
              'px ' + (15 * ratio) + 'px; }';
          html += '#sincloBox #chatTalk li.sinclo_re div.formArea { padding: ' +
              (10 * ratio) + 'px ' + (10 * ratio) + 'px 0 ' + (10 * ratio) +
              'px; margin-top: ' + (10 * ratio) + 'px; }';
          html += '#sincloBox #chatTalk li.sinclo_re div.formElement.withMB { margin-bottom: ' +
              (8 * ratio) + 'px; }';
          html += '#sincloBox #chatTalk li.sinclo_re div.formElement label.formLabel { margin-bottom: ' +
              (3 * ratio) + 'px; }';
          html += '#sincloBox #chatTalk li.sinclo_re div.formElement input.formInput { padding: ' +
              (5 * ratio) + 'px; border: ' + (1 * ratio) + 'px solid ' +
              chatPosition.re.backgroundColor + '; }';
          html += '#sincloBox #chatTalk li.sinclo_re div.formArea p.formOKButtonArea { margin-top: ' +
              (20 * ratio) + 'px; margin-bottom: ' + (20 * ratio) + 'px; }';
          html += '#sincloBox #chatTalk li.sinclo_re div.formArea p.formOKButtonArea span.formOKButton { width: ' +
              (100 * ratio) + 'px; height: ' + (30 * ratio) +
              'px; border-radius: ' + (12 * ratio) + 'px; }';

          html += '#sincloBox #chatTalk li.sinclo_se.sinclo_form { padding: ' +
              (15 * ratio) + 'px; }';
          html += '#sincloBox #chatTalk li.sinclo_se div.formContentArea { margin: 0; line-height: 0; }';
          html += '#sincloBox #chatTalk li.sinclo_se div.formArea { padding: ' +
              (10 * ratio) + 'px ' + (10 * ratio) + 'px 0 ' + (10 * ratio) +
              'px; }';
          html += '#sincloBox #chatTalk li.sinclo_se div.formElement.withMB { margin-bottom: ' +
              (8 * ratio) + 'px; }';
          html += '#sincloBox #chatTalk li.sinclo_se div.formElement label.formLabel { margin-bottom: ' +
              (3 * ratio) + 'px; }';
          html += '#sincloBox #chatTalk li.sinclo_se div.formElement input.formInput { padding: ' +
              (5 * ratio) + 'px; border: ' + (1 * ratio) + 'px solid ' +
              chatPosition.se.backgroundColor + '; }';
          html += '#sincloBox #chatTalk li.sinclo_se div.formArea p.formOKButtonArea { margin-top: ' +
              (20 * ratio) + 'px; margin-bottom: ' + (20 * ratio) + 'px; }';
          html += '#sincloBox #chatTalk li.sinclo_se div.formArea p.formOKButtonArea span.formOKButton.disabled { opacity: 0.38; }';

          html += '#sincloBox section#chatTab sinclo-div#miniFlexBoxHeight { height: ' +
              (48 * ratio) + 'px;  }';
          html += '#sincloBox section#chatTab #sincloChatSendBtn, #sincloBox section#chatTab #miniSincloChatSendBtn { padding:  ' +
              (16.5 * ratio) + 'px 0; border: ' + ratio + 'px solid ' +
              colorList['chatSendBtnBackgroundColor'] + '; }';
          html += '#sincloBox section#chatTab a#miniSincloChatSendBtn { padding: ' +
              (8 * ratio) + 'px 0;  }';
          html += '#sincloBox section#chatTab sinclo-div #sincloChatSendBtn, #sincloBox section#chatTab sinclo-div #miniSincloChatSendBtn { font-size: ' +
              (13 * ratio) + 'px;}';
          html += '#sincloBox section#chatTab sinclo-chat-alert { left: 10%; right: 10%; bottom: 50%; border-radius: ' +
          ...
          html += '#sincloBox section#navigation ul li { padding: ' +
              (10 * ratio) + 'px 0; border-left: ' + (1 * ratio) +
              'px solid #E8E7E0; height: ' + (40 * ratio) + 'px;  }';
          html += '#sincloBox section#navigation ul li:last-child { border-right: ' +
              (1 * ratio) + 'px solid ' + colorList['widgetBorderColor'] +
              '; }';
          if (colorList['widgetBorderNone'] === 1) {
            html += '#sincloBox section#navigation ul li:last-child { border-right:none; }';
          }
          html += '#sincloBox section#navigation ul li:not(.selected) { border-bottom: ' +
              (1 * ratio) + 'px solid ' + colorList['widgetInsideBorderColor'] +
              '; }';
          if (colorList['widgetInsideBorderNone'] === 1) {
            html += '#sincloBox section#navigation ul li:not(.selected) { border-bottom: none!important;}';
          }
          html += '#sincloBox section#navigation ul li.selected::after { border-bottom: ' +
              (2 * ratio) + 'px solid ' + colorList['mainColor'] + '; left: ' +
              (5 * ratio) + 'px; }';
          html += '#sincloBox #fotter { padding: ' + (5 * ratio) +
              'px 0; border: ' + (1 * ratio) + 'px solid ' +
              colorList['widgetBorderColor'] + '; font-size: ' + (11 * ratio) +
              'px; border-top: none;}';
          if (colorList['widgetBorderNone'] === 1) {
            html += '#sincloBox #fotter { border:none; }';
          }
          html += '#sincloBox section#navigation ul li::before { margin-right: ' +
              (5 * ratio) + 'px; width: ' + (18 * ratio) + 'px; height: ' +
              (18 * ratio) + 'px; }';
          /* Cogmo */
          html += '#sincloBox #chatTalk li.sinclo_re p.sincloButtonWrap { padding: ' +
              (10 * ratio) + 'px; border-radius: ' + (12 * ratio) +
              'px; margin: ' + (5 * ratio) + 'px 0px; }';
          html += '#sincloBox #chatTalk li.sinclo_re p.sincloButtonWrap:hover { opacity: 0.8 }';
          html += '#sincloBox #chatTalk li.sinclo_re p.sincloButtonWrap span.sincloButton { font-size: ' +
              (12 * ratio) + 'px;}';
          //閉じるボタン設定が有効かつバナー表示設定になっているかどうか
          if (Number(widget.closeButtonSetting) === 2 &&
              Number(widget.closeButtonModeType) === 1) {
            ...
            if (window.sincloInfo.widget.spBannerPosition) {
              ...
              if (typeof (widget.spBannerHorizontalPosition) === 'string' &&
                  widget.spBannerHorizontalPosition.indexOf('px') !== -1) {
                horizontalPosition = parseInt(
                    widget.spBannerHorizontalPosition) * ratio + 'px';
              ...
              if (typeof (widget.spBannerVerticalPositionFromBottom) ===
                  'string' &&
                  widget.spBannerVerticalPositionFromBottom.indexOf('px') !==
                  -1) {
                verticalPositionFromBottom = parseInt(
                    widget.spBannerVerticalPositionFromBottom) * ratio + 'px';
              } else if (typeof (widget.spBannerVerticalPositionFromBottom) ===
                  'string') {
              ...
              switch (Number(window.sincloInfo.widget.spBannerPosition)) {
                case 1://右下
                  if (check.hasCustomBannerImageSetting()) {
                    html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner { position: fixed; right: ' +
                        horizontalPosition + '; bottom: ' +
                        verticalPositionFromBottom + '; }';
                  } else {
                    html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner { position: fixed; right: ' +
                        horizontalPosition + '; bottom: ' +
                        verticalPositionFromBottom + '; height: ' + (30 * ratio) +
                        'px; box-shadow: 0px 0px ' + widget.boxShadow * ratio +
                        'px ' + widget.boxShadow * ratio +
                        'px rgba(0,0,0,0.1); border-radius: ' +
                        widget.radiusRatio * ratio + 'px ' + widget.radiusRatio *
                        ratio + 'px ' + widget.radiusRatio * ratio + 'px ' +
                        widget.radiusRatio * ratio + 'px; }';
                  }
                  html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner .bannertext{ color: ' +
                      widget.stringColor + '; font-size: ' + (10 * ratio) +
                      'px; vertical-align: middle; margin-right: ' +
                      (5 * ratio) + 'px; }';
                  html += '      #sincloBox div#sincloBannerBox div#sincloBanner div#sincloBannerText #bannerIcon { width: ' +
                      Math.ceil(21 * ratio) + 'px; height: ' +
                      Math.ceil(21 * ratio) + 'px; opacity: 1; margin: 0px ' +
                      (5 * ratio) + 'px; }';
                  break;
                case 2://左下
                    if(check.hasCustomBannerImageSetting()) {
                      html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner { position: fixed; left: ' +
                          horizontalPosition + '; bottom: ' +
                          verticalPositionFromBottom + '; }';
                    } else {
                      html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner { position: fixed; left: ' +
                  ...
                  break;
                case 3://右中央
                  html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner { position: fixed; right:0; top: ' +
                      widget.spBannerVerticalPositionFromTop +
                      '; transform: translateY(-50%); height: auto; box-shadow: 0px 0px ' +
                      widget.boxShadow * ratio + 'px ' + widget.boxShadow *
                      ratio + 'px rgba(0,0,0,0.1); border-radius:' +
                      widget.radiusRatio * ratio + 'px 0 0 ' +
                      widget.radiusRatio * ratio + 'px; }';
                  html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner .bannertext { margin-left: 0px; margin-right: 0; color: ' +
                      widget.stringColor + '; font-size: ' + (10 * ratio) +
                      'px; vertical-align: middle; -webkit-writing-mode: vertical-rl; -ms-writing-mode: tb-rl; writing-mode: vertical-rl; letter-spacing: 1px;}';
                  html += '      #sincloBox div#sincloBannerBox #sincloBanner .sincloBannerText{ flex-flow: column nowrap; margin: ' +
                      Math.ceil(10 * ratio) + 'px ' + Math.ceil(5 * ratio) +
                      'px; }';
                  html += '      #sincloBox div#sincloBannerBox div#sincloBanner div#sincloBannerText #bannerIcon { width: ' +
                      Math.ceil(10 * ratio) + 'px; height: ' +
                      Math.ceil(10 * ratio) +
                      'px; opacity: 1; margin-bottom: 5px; }';
                  break;
                case 4://左中央
                  html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner { position: fixed; left:0; top: ' +
                      widget.spBannerVerticalPositionFromTop +
                      '; transform: translateY(-50%); height: auto; box-shadow: 0px 0px ' +
                      widget.boxShadow * ratio + 'px ' + widget.boxShadow *
                      ratio + 'px rgba(0,0,0,0.1); border-radius:0 ' +
                      widget.radiusRatio * ratio + 'px ' + widget.radiusRatio *
                      ratio + 'px 0; }';
                  html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner .bannertext { margin-right: 0px; margin-left: 0; color: ' +
                      widget.stringColor + '; font-size: ' + (10 * ratio) +
                      'px; vertical-align: middle; -webkit-writing-mode: vertical-rl; -ms-writing-mode: tb-rl; writing-mode: vertical-rl; letter-spacing: 1px;}';
                  html += '      #sincloBox div#sincloBannerBox #sincloBanner .sincloBannerText{ flex-flow: column nowrap; margin: ' +
                      Math.ceil(10 * ratio) + 'px ' + Math.ceil(5 * ratio) +
              ...
              if (Number(window.sincloInfo.widget.showPosition) === 1) {
                ...
                html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner { position: fixed; right: ' +
                    (5 * ratio) + 'px; bottom: ' + (5 * ratio) +
                    'px; height: ' + (30 * ratio) + 'px; box-shadow: 0px 0px ' +
                    widget.boxShadow * ratio + 'px ' + widget.boxShadow *
                    ratio + 'px rgba(0,0,0,0.1); border-radius: ' +
                    widget.radiusRatio * ratio + 'px ' + widget.radiusRatio *
                    ratio + 'px ' + widget.radiusRatio * ratio + 'px ' +
            ...
            html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner .sinclo-comment{ font-size: ' +
                (17.5 * ratio) + 'px; padding: 0 ' + (2 * ratio) + 'px 0 ' +
                (10 * ratio) + 'px; }';
            html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner .sinclo-comment-notext{ font-size: ' +
                (17.5 * ratio) + 'px; padding: 0 ' + (2 * ratio) + 'px 0 ' +
                (13 * ratio) + 'px; }';
        ...
        /* 横の場合 */
          ...
          if (widget.chatMessageCopy === 1) {
            html += '#sincloBox p#widgetTitle { border-radius: 0; border-top-width: 0.1em; height: 2em; padding: 0.3em 2em 0; font-size: 1em;  user-select: none; -moz-user-select: none; -webkit-user-select: none; -ms-user-select: none; }';
          } else {
            html += '#sincloBox p#widgetTitle { border-radius: 0; border-top-width: 0.1em; height: 2em; padding: 0.3em 2em 0; font-size: 1em;}';
          }
          if (widget.widgetSizeType !== 1) {
            html += '#sincloBox p#widgetTitle { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }';
          }
          ...
          html += '#sincloBox ul#chatTalk { padding: 0.3em; padding-top:0px; background-color: ' +
              colorList['chatTalkBackgroundColor'] + '}';
          html += '#sincloBox ul#chatTalk li { font-size: 0.8em; border-radius: 0.72em; margin: 0.3em 0; padding: ' +
              (10 * (ratio / 2)) + 'px ' + (15 * (ratio / 2)) + 'px; }';
          html += '#sincloBox ul#chatTalk li div.sendFileThumbnailArea { display: table-cell; width: ' +
              (64 * ratio) + 'px; height: ' + (64 * ratio) +
              'px; border: 1px solid #D9D9D9; }';
          html += '#sincloBox ul#chatTalk li.sinclo_se { font-size: 0.8em; margin-left: ' +
              (widgetWidth / 342.5) * 45 + 'px;}';
          html += '#sincloBox ul#chatTalk li.sinclo_re { font-size: 0.8em; margin-right: ' +
              (widgetWidth / 342.5) * 21 + 'px;}';
          ...
          if (hRatio > 16) {
            html += '#sincloBox #sincloChatMessage, #sincloBox #miniSincloChatMessage { height: 100%;  min-height: 100%!important; border-radius: 5px 0 0 5px!important; font-size: 0.7em }';
          } else {
          ...
          if (Number(widget.closeButtonSetting) === 2 &&
            ...
            if (Number(window.sincloInfo.widget.showPosition) === 1) {
              html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner { position: fixed; right: ' +
                  (5 * ratio) + 'px; bottom: ' + (5 * ratio) + 'px; height: ' +
                  (30 * ratio) + 'px; box-shadow: 0px 0px ' + widget.boxShadow *
                  ratio + 'px ' + widget.boxShadow * ratio +
                  'px rgba(0,0,0,0.1); border-radius: ' + widget.radiusRatio *
                  ratio + 'px ' + widget.radiusRatio * ratio + 'px ' +
                  widget.radiusRatio * ratio + 'px ' + widget.radiusRatio *
                  ratio + 'px; }';
            ...
            html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner .sinclo-comment-notext{ font-size: ' +
                (17.5 * ratio) + 'px; padding: 0 ' + (2 * ratio) + 'px 0 ' +
                (13 * ratio) + 'px; }';
            html += '      #sincloBox div#sincloBannerBox div#sincloBanner #bannerIcon { width: ' +
                Math.ceil(21 * ratio) + 'px; height: ' + Math.ceil(21 * ratio) +
                'px; opacity: 1; margin: 0px ' + (5 * ratio) + 'px; }';
            html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner .bannertext{ color: ' +
                widget.stringColor + '; font-size: ' + (10 * ratio) +
                'px; vertical-align: middle; margin-right: ' + (5 * ratio) +
                'px; }';
            html += '      #sincloBox div#sincloBannerBox div#sincloBanner.sincloBanner .notext{ cursor: pointer; }';
          }
        }
      ...
      /* PC版 */
        ...
        html += '      #sincloBox section#navigation ul { width: ' +
            sizeList['boxWidth'] + 'px }';
        html += '      #sincloBox span#mainImage { top: 7px; left: 8px }';
        html += '      #sincloBox span#mainImage i {display: flex; justify-content: center; align-items: center; width: 80px; height: 70px; font-size: calc(43px * ((3 * ' +
            widget.headerTextSize + ' + 36) / 81)); border: 1px solid; }';
        ...
        html += '      #sincloBox p#widgetTitle.centerPosition.noImage { padding-left: 0px; padding-right: 0px; }';
        html += '      #sincloBox p#widgetTitle #sincloChatUnread { width: 25px; height: 25px; font-size: ' +
            (widget.headerTextSize - 1) +
            'px; border-radius: 15px; margin: 2.5px 6px; padding: 3px; }';
        html += '      #sincloBox p#widgetTitle:after { background-position-y: 3px; top: ' +
            sizeList['widgetTitleTop'] +
            'px; right: 10px; bottom: 6px; width: 20px; height: 20px; }';

        html += '      #sincloBox p#widgetSubTitle { background-color: ' +
            colorList['headerBackgroundColor'] +
            '; margin: 0; text-align: left; border-width: 0 1px 0 1px; padding-top: 3px; padding-bottom: 3px; border-color: ' +
            colorList['widgetBorderColor'] +
            '; border-style: solid; font-weight: bold; color: ' +
            colorList['subTitleTextColor'] +
            '; height: auto; line-height:calc(1em + 9px); font-size: ' +
            (Number(widget.headerTextSize) - 2) +
            'px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;}';
        ...
        html += '      #sincloBox p#widgetSubTitle.centerPosition.noImage { padding-left: 0px; padding-right: 0px;}';
        html += '      #sincloBox p#widgetSubTitle.oneContent { line-height: calc((1em + 9px)*2); border-bottom: 1px ' +
            colorList['widgetBorderColor'] + ' solid;}';
        if (colorList['widgetInsideBorderNone'] === 1) {
          html += '      #sincloBox p#widgetSubTitle.oneContent{ border:none;}';
        }
        ...
        html += '      #sincloBox p#widgetDescription { background-color: ' +
            colorList['headerBackgroundColor'] +
            '; margin: 0; padding-bottom: 7px; text-align: left; border-width: 0 1px 1px 1px; border-color: ' +
            colorList['widgetBorderColor'] +
            '; border-style: solid; height: auto; line-height:calc(1em + 2px); color: ' +
            colorList['descriptionTextColor'] + '; border-bottom-color:' +
            colorList['widgetInsideBorderColor'] + '; font-size: ' +
            (Number(widget.headerTextSize) - 2) +
            'px; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;;}';
        if (colorList['widgetBorderNone'] === 1) {
          html += '#sincloBox p#widgetDescription { border-left:none; border-right:none;}';
        }
        html += '      #sincloBox section { background-color: #FFF; border: 1px solid ' +
            colorList['widgetBorderColor'] +
        ...
        html += '      #sincloBox ul#chatTalk li.sinclo_etc { border: none; text-align: center!important; margin: 0 auto; font-weight: bold; font-size:calc(' +
            widget.reTextSize + 'px*0.92); }';
        html += '      #sincloBox section#chatTab sinclo-div #sincloChatSendBtn, #sincloBox section#chatTab sinclo-div #miniSincloChatSendBtn { font-size: ' +
            widget.chatSendBtnTextSize + 'px;}';

        // チャットを使用する際
        if (window.sincloInfo.contract.chat) {
          html += '      #sincloBox #mainImage em { width: 25px; height: 20px; font-size: ' +
              sizeList['d11font'] + 'px; padding: 1px; margin-top: calc(' +
              (-80 * ratio) + 'px* ((3 * ' + widget.headerTextSize +
              ' + 36) / 81 ))!important}';
          html += '      #sincloBox #mainImage em { margin-left:calc(' +
              (48 * ratio) + 'px* ((3 * ' + widget.headerTextSize +
              ' + 36) / 81))!important}';
          html += '      #sincloBox ul#chatTalk { height: ' +
              sizeList['chatTalkHeight'] +
              'px; padding: 0px 5px 0px 5px; background-color: ' +
              colorList['chatTalkBackgroundColor'] + ' }';
          html += '      #sincloBox ul#chatTalk li { border-radius: 12px; margin: 10px 0 0 0; padding: 10px 15px; }';
          if (colorList['seBorderNone'] === 0) {
            html += '      #sincloBox ul#chatTalk li.sinclo_se { border: 1px solid ' +
                chatPosition.se.borderColor + '; }';
          }
          if (colorList['reBorderNone'] === 0) {
            html += '      #sincloBox ul#chatTalk li.sinclo_re { border: 1px solid ' +
                chatPosition.re.borderColor + '; }';
          }
          html += '      #sincloBox ul#chatTalk li span.cName { font-size: ' +
              (Number(widget.reTextSize)) + 'px; margin: 0 0 5px 0 }';
          html += '      #sincloBox section#chatTab sinclo-div:not(#flexBoxWrap) { height: ' +
              sizeList['classFlexBoxRowHeight'] +
              'px!important; padding: 5px }';
          ...
          html += '      #sincloBox section#chatTab #sincloChatMessage, #sincloBox section#chatTab #miniSincloChatMessage { color: ' +
              chatContentTextColor +
              '!important; padding: 5px; height: 100%; min-height: 100%!important; }';
          if (colorList['messageBoxBorderNone'] === 0) {
            html += '      #sincloBox section#chatTab #sincloChatMessage, #sincloBox section#chatTab #miniSincloChatMessage { border: 1px solid ' +
                colorList['messageBoxBorderColor'] +
                '!important; border-radius: 5px 0 0 5px!important; }';
          } else {
            html += '      #sincloBox section#chatTab #sincloChatMessage, #sincloBox section#chatTab #miniSincloChatMessage { border: none!important;}';
          }
          html += '      #sincloBox section#chatTab #sincloChatSendBtn, #sincloBox section#chatTab #miniSincloChatSendBtn { padding: 20px 0; height: 100%; border: 1px solid ' +
              colorList['chatSendBtnBackgroundColor'] + '; }';
          html += '      #sincloBox section#chatTab #miniSincloChatSendBtn { padding: 8px 0; height: 100%; border: 1px solid ' +
              colorList['chatSendBtnBackgroundColor'] + '; }';
        }
        // 画面同期を使用する際
        if (window.sincloInfo.contract.synclo ||
            (window.sincloInfo.contract.hasOwnProperty('document') &&
                window.sincloInfo.contract.document)) {
          html += '      #sincloBox section#callTab { background-color: ' +
              colorList['chatTalkBackgroundColor'] + '!important; }';
          html += '      #sincloBox section#callTab #telNumber { overflow: hidden; color: ' +
              colorList['mainColor'] +
              '; font-weight: bold; margin: 0 auto; text-align: center; border: none!important; overflow: visible!important; }';
          html += '      #sincloBox section#callTab #telIcon { color: ' +
              colorList['mainColor'] +
              '; display: block; width: 50px; height: 50px; float: left; background-color: #3EA3DE; border-radius: 25px; padding: 3px }';
          html += '      #sincloBox section#callTab #telContent { display: block; overflow-y: auto; overflow-x: hidden; height:' +
              sizeList['telContentHeight'] + 'px; max-height: ' +
              sizeList['telContentHeight'] + 'px }';
          if (window.sincloInfo.contract.chat) {
          ...
          html += '      #sincloBox section#callTab #telContent span { word-wrap: break-word ;word-break: break-all; font-size: ' +
              sizeList['d11font'] +
              'px; line-height: 1.5!important; color: #6B6B6B; white-space: pre-wrap; max-height: 119px; display: table-cell; vertical-align: middle; text-align: center }';
          if (window.sincloInfo.contract.chat) {
            html += '      #sincloBox section#callTab #accessIdArea { height: 50px; display: block; margin: 20px auto 21px; width: 80%; padding: 7px;  color: #FFF; background-color: rgb(188, 188, 188); font-size: ' +
                sizeList['d25font'] +
                'px; font-weight: bold; text-align: center; border-radius: 15px } ';
          } else {
            html += '      #sincloBox section#callTab #accessIdArea { height: 50px; display: block; margin: 10px auto; width: 80%; padding: 7px;  color: #FFF; background-color: rgb(188, 188, 188); font-size: ' +
                sizeList['d25font'] +
                'px; font-weight: bold; text-align: center; border-radius: 15px } ';
          }
        }
        html += '      #sincloBox section#navigation { border-width: 0 1px; height: ' +
            sizeList['navigationHeight'] + 'px; }';
        html += '      #sincloBox section#navigation ul { margin: 0 0 0 -1px; height: ' +
            sizeList['navigationHeight'] + 'px;}';
        html += '      #sincloBox section#navigation ul li { width: 50%; padding: 10px 0; border-left: 1px solid ' +
            colorList['widgetBorderColor'] + '; height: ' +
            sizeList['navigationHeight'] + 'px }';
        if (colorList['widgetBorderNone'] === 1) {
          html += '      #sincloBox section#navigation ul li { border-left:none; }';
        }
        html += '      #sincloBox section#navigation ul li:last-child { border-right: 1px solid ' +
            colorList['widgetBorderColor'] + '; border-left: 1px solid ' +
            colorList['widgetInsideBorderColor'] + '; }';
        if (colorList['widgetBorderNone'] === 1) {
          html += '      #sincloBox section#navigation ul li:last-child { border-right:none; }';
        }
        ...
        html += '      #sincloBox section#navigation ul li:not(.selected) { border-bottom: 1px solid ' +
            colorList['widgetInsideBorderColor'] + ' }';
        if (colorList['widgetInsideBorderNone'] === 1) {
          html += '      #sincloBox section#navigation ul li:not(.selected) { border-bottom: none!important; }';
        }
        html += '      #sincloBox section#navigation ul li.selected::after{ border-bottom: 2px solid ' +
            colorList['mainColor'] + '; }';
        html += '      #sincloBox #fotter { height: ' +
            sizeList['fotterHeight'] +
            'px; padding: 5px 0; border: 1px solid ' +
            colorList['widgetBorderColor'] + '; font-size: ' +
            sizeList['d11font'] + 'px; border-top: none; }';
        if (colorList['widgetBorderNone'] === 1) {
          html += '      #sincloBox #fotter { border:none }';
        }
    ...
    sincloBannerTemplate: function(widget) {
      ...
      if (check.hasCustomBannerImageSetting()) {
        if (check.smartphone()) {
          html += '<img src="' + window.sincloInfo.custom.widget.bannerImage.sp +'" style="width:150px; height:90px" />';
        } else {
        ...
        html += '      <svg version="1.1" id="bannerIcon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" xml:space="preserve">\n' +
            ...
            '\t\tC376.619,414.89,512,366.282,512,217.458C512,102.036,418.974,19.179,257.135,19.179z" style="fill: ' +
            common.toRGBCode(sincloInfo.widget.stringColor) + ';"></path>\n' +
            '</g>\n' +
            '</svg>';
        var bannertext = '';
        ...
        html += '      <span class="' +
            (bannertext.length !== 0 ? 'bannertext' : 'notext') + '">' +
            bannertext + '</span>';
      }
      html += '    </div>';
      html += '  </div>';
    ...
    widgetCallOpTemplate: function(widget) {
      ...
      if (check.isset(sincloInfo.custom)
        ...
        html += '    <span id="callOperatorButton" onclick="sinclo.chatApi.callOperator()"><span id="callOpButtonIconWrap"><img src="data:image/png;base64," id="callOpButtonIcon" alt="オペレータ呼び出し"/></span><span id="callOpButtonLabel">オペレータ呼び出し</span></span>';
        html += '  </section>';
      }
      return html;
    },
    injectCallOpButtonCSS: function(widget) {
      ...
      return navicss +
          ...
          'width: 100%; ' +
          'background-color: ' + widget.mainColor + ';' +
          'margin: 0 ' + (5 * ratio) + 'px; ' +
          'display: block;' +
          'height: ' + (44 * ratio) + 'px;' +
          ...
          'display: inline-flex;' +
          '}' +
          '#sincloBox #navigation span#callOperatorButton img#callOpButtonIcon {' +
          'width: ' + (32 * ratio) + 'px;' +
          'height: ' + (32 * ratio) + 'px;' +
          '}' +
    ...
    widgetTemplate: function(widget) {
      ...
      // アイコン
      html += '    <span style="display: block; width: 50px; height: 50px; float: left; background-color: ' +
          colorList['mainColor'] +
          '; border-radius: 25px; padding: 3px;"><img width="19.5" height="33" src="' +
          window.sincloInfo.site.files +
          '/img/call.png" style="width: 19.5px; height: 33px; margin: 6px 12px"></span>';
      // 受付電話番号
      if (Number(widget.display_time_flg) === 1) {
        html += '    <pre id="telNumber" style="font-size: ' +
            sizeList['d18font'] + 'px; padding: 5px 0px 0px; height: 30px">' +
            check.escape_html(widget.tel) + '</pre>';
      } else {
        html += '    <pre id="telNumber" style="font-size: ' +
            sizeList['d20font'] + 'px; padding: 10px 0px 0px; height: 45px;">' +
            check.escape_html(widget.tel) + '</pre>';
      }
      // 受付時間
      if (Number(widget.display_time_flg) === 1) {
        html += '    <pre style="font-weight: bold; color: ' +
            colorList['mainColor'] +
            '; margin: 0 auto; white-space: pre-line; font-size: 11px; text-align: center; padding: 0 0 5px; height: 20px; border: none!important;  overflow: visible!important;">受付時間： ' +
            widget.time_text + '</pre>';
      }
      html += '    </sinclo-div>';
      // テキスト
      html += '    <sinclo-div id="telContent"><sinclo-div class="tblBlock"><span>' +
          check.escape_html(widget.content) +
          '</span></sinclo-div></sinclo-div>';
    ...
    showVideoChatView: function(fromID, toID) {
      ...
      iframe.src = sincloInfo.site.webcam_view + '?h=false&sincloData=' +
          encodeURIComponent(JSON.stringify(sincloData)); // FIXME
    ...
    chatWidgetTemplate: function(widget) {
      ...
      if (!check.smartphone() && (window.sincloInfo.contract.synclo ||
          (window.sincloInfo.contract.hasOwnProperty('document') &&
              window.sincloInfo.contract.document) ||
          (check.isset(widget.showAccessId) && widget.showAccessId === 1))) {
        html += '    <span id="sincloAccessInfo">ウェブ接客コード：' +
            userInfo.accessId + '</span>';
      }
      html += '    <audio id="sinclo-sound">';
      html += '      <source src="' + window.sincloInfo.site.files +
          '/sounds/decision.mp3" type="audio/mp3">';
      html += '    </audio>';
      html += '  </section>';
      return html;
    },
    judgeShowWidget: function() {
      ...
      // ウィジェットを常に表示する
      if (('display_type' in window.sincloInfo.widget) &&
          window.sincloInfo.widget.display_type === 1) {
        window.sincloInfo.widgetDisplay = true;
      }
      // オペレーターの数に応じて表示する
      else if (('display_type' in window.sincloInfo.widget) &&
          window.sincloInfo.widget.display_type === 2) {
      ...
      if (check.isset(userInfo.connectToken) ||
          check.isset(userInfo.coBrowseConnectToken)) {
        window.sincloInfo.widgetDisplay = false;
      }

      // 同時対応上限数の設定があり、超えている場合
      if (window.sincloInfo.hasOwnProperty('opFlg') &&
          window.sincloInfo.opFlg === false) {
        window.sincloInfo.widgetDisplay = false;
      }
      // 常に表示する設定の値があれば必ず表示する
      if (check.isset(window.sincloInfo.dataset) &&
          (check.isset(window.sincloInfo.dataset.showAlways) &&
              window.sincloInfo.dataset.showAlways === '1')) {
        window.sincloInfo.widgetDisplay = true;
      }
      // 同期対象とするが、ウィジェットは表示しない
      if (check.isset(window.sincloInfo.dataset) &&
          (check.isset(window.sincloInfo.dataset.hide) &&
              window.sincloInfo.dataset.hide === '1')) {
        window.sincloInfo.widgetDisplay = false;
      }
    ...
    makeAccessIdTag: function() {
      ...
      if (userInfo.accessType !== cnst.access_type.host) {
        ...
        sinclo.operatorInfo.header = document.querySelector(
            '#sincloBox #widgetHeader');

        $('#sincloBox .widgetCtrl').click(function() {
          var target = $('#sincloBox .widgetCtrl.selected'),
              clickTab = $(this).data('tab');
...
//                sinclo.operatorInfo.onBanner();
    ...
    toRGBAcolor: function(colorCode, opacity) {
      if (colorCode.indexOf('#') >= 0) {
        ...
        colorCode = 'rgba(' + parseInt(r, 16) + ', ' + parseInt(g, 16) + ', ' +
            parseInt(b, 16) + ', ' + opacity + ')';
      }
      return colorCode;
    },
...
//          storage.s.unset("closeAct");
        ...
        if ((common.widgetHandler.isShown() || window.sincloInfo.widgetDisplay)
          ...
          // テキストエリアの表示非表示
          if (!storage.l.get('textareaOpend') ||
              storage.l.get('textareaOpend') === 'open') {
          ...
          if (!reCreateWidget && dataOpenflg === 'false') {
            ...
            if ((!(check.smartphone() &&
                ...
                storage.s.get('preWidgetOpened') === 'true')
                || (!check.smartphone() && storage.s.get('preWidgetOpened') ===
                    'true')) {
      ...
      saveShownFlg: function() {
        ...
        storage.s.set('widgetShowTimingType',
            window.sincloInfo.widget.showTiming);
      },
      ...
      _calcRemainingShowTimingSiteTimeMsec: function() {
        var siteAccessTimeMsec = (Number(userInfo.pageTime) -
            Number(userInfo.time)),
            showIntervalMsec = window.sincloInfo.widget.maxShowTimingSite *
                1000;
        console.log('siteAccessTimeMsec ' + siteAccessTimeMsec);
        console.log('showIntervalMsec' + showIntervalMsec);
        console.log('_calcRemainingShowTimingSiteTimeMsec: ' +
            (siteAccessTimeMsec <= showIntervalMsec ?
                showIntervalMsec - siteAccessTimeMsec :
                0));
        return siteAccessTimeMsec <= showIntervalMsec ?
            showIntervalMsec - siteAccessTimeMsec :
            0;
      },
      ...
      _setFooterSize: function() {
        if (check.isset(window.sincloInfo.custom) &&
            check.isset(window.sincloInfo.custom.widget) &&
            check.isset(window.sincloInfo.custom.widget.hideFotter) &&
            window.sincloInfo.custom.widget.hideFotter) {
          return 0;
        } else {
      ...
      _maximumAnimation: function() {
        ...
        var self = common.widgetHandler;
        var footerSize = self._setFooterSize();
        var inputAreaSize = self._setInputAreaSize();
        var offset = $('#widgetHeader').outerHeight() +
      ...
      _handleResizeEvent: function() {
        ...
        if (delta > 0 && afterWidgetHeight > maxCurrentWidgetHeight) {
          console.log('<><><><><><><><><>1 %s %s %s', delta, afterWidgetHeight,
              maxCurrentWidgetHeight);
          changeTarget.height(common.widgetHandler._getMaxChatTalkHeight());
        } else if (delta < 0 && afterWidgetHeight < minCurrentWidgetHeight) {
          console.log('<><><><><><><><><>2 %s %s %s', delta, afterWidgetHeight,
              minCurrentWidgetHeight);
          changeTarget.height(common.widgetHandler._getMinChatTalkHeight());
        } else if ((delta < 0 && windowHeight * 0.85 < currentWidgetHeight) ||
            (delta > 0 && windowHeight * 0.85 >= afterWidgetHeight)) {
          console.log('<><><><><><><><><>3 %s %s %s %s', delta, windowHeight,
              currentWidgetHeight, afterWidgetHeight);
      ...
      _getMessageAreaOffset: function(forChatTalkOffset) {
        ...
        if (!forChatTalkOffset) {
          ...
          if (!$('#widgetSubTitle').is(':visible') &&
              !$('#widgetDescription').is(':visible')) {
    ...
    chatBotTyping: function(obj) {
      ...
      if (!check.smartphone() && widget.widgetSizeType === 1 &&
          $('#chatTalk').get(0).offsetHeight <
          $('#chatTalk').get(0).scrollHeight) {
      ...
      if (widget.chatMessageWithAnimation === 1) {
        if (window.sincloInfo.widget.isSendMessagePositionLeft) {
          ...
          css += "#sincloBox ul#chatTalk div.botNowDiv { text-align: left; }";
    ...
    reloadWidget: function() {
      ...
      var coverHeight = $('#chatTalk').outerHeight() +
          $('#flexBoxHeight').outerHeight();
      ...
      css += '  background-color:' + widget.chatTalkBackgroundColor + ';';
      css += '  width:' + coverWidth + 'px;height:' + coverHeight +
          'px;padding:0 ' + loadPadding + 'px;';
      css += '}';
      css += '@keyframes dotScale{';
  ...
  check = {
    browser: function() {
      ...
      if (window.localStorage && window.sessionStorage) {
        if (!check.isset(common.tmpParams) &&
            !check.isset(storage.s.get('params'))) {
          ret = true;
        }
      }
      var ua = navigator.userAgent.toLowerCase();
    ...
    hasCustomBannerImageSetting: function() {
      return check.isset(window.sincloInfo.custom) &&
          check.isset(window.sincloInfo.custom.widget) &&
          check.isset(window.sincloInfo.custom.widget.bannerImage) &&
          ((!check.smartphone() &&
              check.isset(window.sincloInfo.custom.widget.bannerImage.pc)) ||
              (check.smartphone() &&
                  check.isset(window.sincloInfo.custom.widget.bannerImage.sp)));
    },
    // エスケープ用
  ...
  userInfo = {
    ...
    init: function() {
      ...
      if (check.isset(storage.s.get('params'))) {
        ...
        if (Number(common.tmpParams.type) === Number(cnst.access_type.host)) {
          ...
          if (check.isset(userInfo.referrer)) {
            var ret = userInfo.referrer.match(
                /[?&](kw|MT|name|p|q|qt|query|search|word)=([^&]+)/);
            userInfo.searchKeyword = (check.isset(ret) && ret.length === 3) ?
                ret[2] :
                null;
          }
        }
      ...
      if (Number(common.params.type) === Number(cnst.access_type.host)) {
        ...
        // 消費者がフレームの場合
        if (common.tmpParams.hasOwnProperty('gFrame') &&
            !check.isset(storage.s.get('gFrame'))) {
        ...
        if (check.isset(storage.s.get('gFrame')) &&
            check.isset(storage.s.get('parentId'))) {
    ...
    setPrevpage: function(reset) {
      ...
      // IE8対応コード
      if (userInfo.prev.length === 0 || location.href !==
          userInfo.prev[userInfo.prev.length - 1].url) {
    ...
    writePrevToLocalStorage: function() {
      ...
      if (typeof (userInfo.oldSincloSessionId) !== 'undefined') {
        console.log(
            'oldSincloSessionId is found. : ' + userInfo.oldSincloSessionId +
            ' overwrite.');
        ...
        try {
          for (var i = 0; i < sincloInfo.customVariable.length; i++) {
            var getValue = userInfo._getText(
                $(sincloInfo.customVariable[i].attribute_value));
            if (getValue) {
  ...
  syncEvent = {
    ...
    evList: [
      ...
      },
      {
        ...
        ev: function(e) {
          ...
          if ('body' === syncEvent.receiveEvInfo.nodeName && 'scroll' ===
              syncEvent.receiveEvInfo.type) return false;
          // スクロール用
          emit('syncScrollInfo', {
            accessType: userInfo.accessType,
            mousePoint: {x: e.clientX, y: e.clientY},
    ...
    ctrlEventListener: function(eventFlg, evList) { // ウィンドウに対してのイベント操作
      ...
      for (var i in evList) {
        var evName = (attachFlg) ?
            'on' + String(evList[Number(i)].type) :
            String(evList[Number(i)].type);
    ...
    changeCall: function(e) {
      ...
      if (nodeName !== 'input' && nodeName !== 'textarea' && nodeName !==
          'select') return false;
      if (e.target.type === 'radio' || e.target.type === 'checkbox') {
        checked = e.target.checked;
      }
      // 排他処理
      if (nodeName === String(syncEvent.receiveEvInfo.nodeName) &&
          Number(index) === Number(syncEvent.receiveEvInfo.idx)) return false;
      emit('syncChangeEv', {
        tabId: userInfo.tabId,
        userId: userInfo.userId,
        accessType: userInfo.accessType,
    ...
    resizeCall: function(ua, eventFlg) {
      ...
      if (Number(userInfo.accessType) !==
          Number(cnst.access_type.guest)) return false;
      if (
        ...
        window.addEventListener('orientationchange', syncEvent.tabletResize,
            false);
      } else {
    ...
    elmScrollCall: function(e) {
      ...
      syncEvent.elmScrollCallTimers[nodeName + '_' + index] = setTimeout(
          function() {
    ...
    change: function(eventFlg) {
      ...
      if ($textarea !== undefined) {
        ...
        $textarea.addEventListener('mousemove', function(e) {
          if (e) e.stopPropagation();
          if (bHeight && bWidth &&
              (bHeight !== this.style.height || bWidth !== this.style.width)) {
          }
      ...
      this.ctrlElmEventListener(eventFlg, scEls, 'scroll',
          syncEvent.elmScrollCall);
      if (('form' in window.sincloInfo.dataset) &&
          window.sincloInfo.dataset.form) {
        // フォーム制御
        $(document).submit(function(e) {
          if (userInfo.accessType !== cnst.access_type.host) {
            emit('requestSyncStopForSubmit',
                {message: 'お客様がsubmitボタンをクリックしましたので、\n画面共有を終了します。'});
          } else {
            e.preventDefault();
            e.stopPropagation();
            return false;
  ...
  popup = {
    ...
    set: function(title, content, type) {
      ...
      html += '      <sinclo-div id="sincloLogo"><img src="' +
          sincloInfo.site.files +
          '/img/mark.png" width="60" height="60"></sinclo-div>';
      html += '      <sinclo-div id="sincloMessage">';
      html += '          <sinclo-h3>' + title +
          ':</sinclo-h3><sinclo-content>' + content + '</sinclo-content>';
      html += '      </sinclo-div>';
      html += '    </sinclo-div>';
  ...
  vcPopup = {
    ...
    set: function(fromID, toID) {
      ...
      var url = sincloInfo.site.webcam_view + '?h=false&sincloData=' +
          encodeURIComponent(JSON.stringify(sincloData));
      html += '  <style>';
      ...
      html += '      <sinclo-div id="sincloVcLogo"><img src="' +
          sincloInfo.site.files +
          '/img/mark.png" width="18" height="18"></sinclo-div>';
      html += '    </sinclo-div>';
      html += '    <sinclo-div id="sincloVcPopMain">';
      html += '      <sinclo-div id="sincloVcMessage">';
      html += '          <iframe id="sincloVcView" src="' + url +
  ...
  var init = function() {
    window.addEventListener('load', function() {
      if ('orientation' in window) {
        ...
        console.log('common.defaultOrientation = %s',
            common.defaultOrientation);
      }
    }, false);
    if (check.smartphone()) {
    ...
    if (window.sincloInfo.widget.showTiming !==
        Number(storage.s.get('widgetShowTimingType'))) {
      // SessionStorageで保存している表示タイミング設定と違う場合はクリアする
      console.log(
          'Storage widgetShowTimingType is different. clearing... before: ' +
          Number(storage.s.get('widgetShowTimingType')) + ' after: ' +
          window.sincloInfo.widget.showTiming);
    ...
    socket.on('disconnect', function(data) {
      if (!sinclo.chatApi.inactiveCloseFlg) {
        ...
        if (sincloBox) {
          common.widgetHandler.hide();
          if (window.sincloInfo.contract.chat &&
              document.getElementsByTagName('sinclo-chat')[0]) {
...
function emit(evName, data, callback) {
  ...
  if (evName === 'syncReady' || evName === 'connectSuccess' || evName ===
      'customerInfo' || evName === 'sendAccessInfo') {
    data.subWindow = false;
    if (check.isset(storage.s.get('params')) || userInfo.accessType ===
        cnst.access_type.host) {
      data.responderId = common.params.responderId;
      data.subWindow = true;
    }
  }
  if (evName === 'syncReady' || evName === 'connectSuccess' || evName ===
      'sendAccessInfo' || evName === 'customerInfo') {
  ...
  if (evName === 'connectSuccess' || evName === 'sendWindowInfo' || evName ===
      ...
      evName === 'getChatMessage' || evName === 'sendChat' || evName ===
      'sendAutoChatMessage'
  ) {
    data.chat = null;
  }
  if (evName === 'syncBrowserInfo' || evName === 'syncChangeEv' || evName ===
      'requestSyncStop' ||
      evName === 'requestSyncStart' || evName === 'connectContinue' ||
      evName === 'sendConfirmConnect'
  ) {
    data.accessType = userInfo.accessType;
  }
  if (evName === 'syncReady' || evName === 'connectSuccess' || evName ===
      'reqUrlChecker' || evName === 'customerInfo' ||
      evName === 'requestSyncStart' || evName === 'connectContinue' ||
      evName === 'sendAccessInfo' || evName === 'sendWindowInfo'
  ) {
    data.url = f_url(browserInfo.href);
  }
  // connectToken
  if (evName === 'syncReady' || evName === 'connectSuccess' || evName ===
      'requestSyncStop' || evName === 'customerInfo' || evName ===
      'sendTabInfo' ||
      evName === 'requestSyncStart' || evName === 'connectContinue' ||
      evName === 'sendAccessInfo' || evName === 'sendConfirmConnect'
  ) {
    data.connectToken = userInfo.get(cnst.info_type.connect);
  }
  if (evName == 'sendWindowInfo' || evName == 'requestSyncStopForSubmit' ||
      evName == 'startSyncToFrame') {
    data.connectToken = userInfo.connectToken;
  }
  if (evName == 'requestSyncStop' && userInfo.accessType ===
      cnst.access_type.host) {
    data.type = 3;
  }
  if (evName == 'requestSyncStop' && userInfo.accessType ===
      cnst.access_type.guest) {
    data.type = 4;
  }
...
function link(word, link, eventLabel) {
  ...
  */
  console.log('ga連携します');
  console.log(
      '押されたやつのテキストは' + word + '値は' + link + 'イベントラベルは' + eventLabel + 'です');
