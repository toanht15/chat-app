...
//    html += '      #sincloBox[data-openflg="false"] p#widgetTitle:after { -ms-transform: rotate(180deg); -moz-transform: rotate(180deg); -webkit-transform: rotate(180deg); -o-transform: rotate(180deg); transform: rotate(180deg); }';
      ...
      if ( window.sincloInfo.contract.chat ) {
        ...
        if (widget.chatMessageDesignType === 2) {
          ...
          // BOX型
          html += '      #sincloBox ul#chatTalk li.sinclo_se { ' + chatPosition.se + 'background-color: '+ colorList['seBackgroundColor'] +'; }';
          html += '      #sincloBox ul#chatTalk li.sinclo_re { ' + chatPosition.re + 'background-color:' + colorList['reBackgroundColor'] + ' }';
        }
        ...
        if(colorList['messageBoxBorderNone'] === 0){
          html += '      #sincloBox section#chatTab sinclo-div #sincloChatMessage { border-right-color: ' + colorList['mainColor'] + '!important; }';
      ...
      html += '      #sincloBox section#navigation ul li { position: relative; overflow: hidden; cursor: pointer; color: ' + colorList['otherTextColor'] + '; text-align: center; display: table-cell; }';
      ...
      html += '      #sincloBox section#navigation ul li.selected::before{ background-color: ' + colorList['mainColor'] + '; }';
      //閉じるボタン設定が有効かつバナー表示設定になっているかどうか
      if(Number(widget.closeButtonSetting) === 2 && Number(widget.closeButtonModeType) === 1){
        ...
        html += '      #sincloBanner.sincloBanner { position: relative; z-index: 1; height: 60px; width : fit-content !important; background-color: '+ colorList['mainColor'] +'; box-shadow: 0px 0px ' + widget.boxShadow + 'px ' + widget.boxShadow + 'px rgba(0,0,0,0.1); border-radius: ' + widget.radiusRatio + 'px ' + widget.radiusRatio + 'px ' + widget.radiusRatio + 'px ' + widget.radiusRatio + 'px; color: '+ colorList['stringColor'] +'; margin: auto; filter:alpha(opacity=80); -moz-opacity: 0.8; opacity: 0.8; cursor: pointer; zoom: 70%; }';
        html += '      #sincloBanner.sincloBannerText{ line-height: 60px; height: auto!important; width: auto!important; padding:0; }';
        html += '      #sincloBanner.sincloBanner i{ color: '+ colorList['stringColor'] +'; }';
...
//        html += '#sincloBox p#widgetTitle:after { background-position-y: ' + (3 * ratio) + 'px; top: ' + (6 * ratio) + 'px; right: ' + (10 * ratio) + 'px; bottom: ' + (6 * ratio) + 'px; width: ' + (20 * ratio) + 'px; height: ' + (20 * ratio) + 'px; }';
          ...
          html += '#sincloBox p#widgetSubTitle { background-color: #FFF; border-color: '+ colorList['widgetBorderColor'] +'; font-weight: bold; color: ' + colorList['subTitleTextColor'] + '; border-style: solid; text-align: left; margin: 0; padding: ' + (7 * ratio) + 'px 0; border-width: 0 ' + (1 * ratio) + 'px 0 ' + (1 * ratio) + 'px; padding-left: ' + (77 * ratio) + 'px; height: ' + (29 * ratio) + 'px; }';
          if(widget.widgetSizeType !== 1){
...
//        html += "      #sincloBox section#navigation ul { width: " + widgetWidth + "px }";
        ...
        // 画面同期を使用する際
        if ( window.sincloInfo.contract.synclo || (window.sincloInfo.contract.hasOwnProperty('document') && window.sincloInfo.contract.document) ) {
          html += '      #sincloBox section#callTab #telNumber { overflow: hidden; color: ' + colorList['mainColor'] + '; font-weight: bold; margin: 0 auto; text-align: center; background-color: #FFF!important; border: none!important; overflow: visible!important; }';
...
//      }
    ...
    widgetTemplate: function(widget){
      ...
      // 受付時間
      if ( Number(widget.display_time_flg) === 1 ) {
          html += '    <pre style="font-weight: bold; color: ' + colorList['mainColor'] + '; margin: 0 auto; white-space: pre-line; font-size: 11px; text-align: center; padding: 0 0 5px; height: 20px; background-color: #FFF!important; border: none!important;  overflow: visible!important;">受付時間： ' + widget.time_text + '</pre>';
      }
      html += '    </sinclo-div>';
      // テキスト
